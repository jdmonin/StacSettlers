 ...
 *         {@link SOCRejectConnection}, or the lists of channels and games
 *         ({@link SOCChannels}, {@link SOCGames}).
 *         <UL>
 *         <LI>See {@link SOCMessage} for details of the client/server protocol.
 *         <LI>See {@link Server} for details of the server threading and
 *         processing.
 *         <LI>To get a player's connection, use {@link #getConnection(Object)
 *         getConnection(plName)}.
 *         <LI>To get a client's nickname, use <tt>(String)</tt>
 *         {@link StringConnection#getData() connection.getData()}.
 *         <LI>To get the rest of a client's data, use ({@link SOCClientData})
 *         {@link StringConnection#getAppData() connection.getAppData()}.
 *         <LI>To send a message to all players in a game, use
 *         {@link #messageToGame(String, SOCMessage)} and related methods.
 *         </UL>
 *         <P>
 ...
 *         {@link #processDebugCommand(StringConnection, String, String)} and
 *         {@link #handleGAMETEXTMSG(StringConnection, SOCGameTextMsg)} for
 *         details.
 *         <P>
 *         The version check timer is set in
 *         {@link SOCClientData#setVersionTimer(SOCServer, StringConnection)}.
...
public class SOCServer extends Server {
    /// fields for collecting gameplay in the database, to be used as traces during
    /// the offline learning procedure
    ...
    /**
     * map for linking a game name to its parameters required for loading or
     * starting with a specific configuration
     */
    ...
    /**
     * Constant for the directory for soclog files. TODO: Make configurable
     */
    public static final String LOG_DIR = "logs_server/";

    /**
     * Default tcp port number 8880 to listen, and for client to connect to remote
     * server. Should match SOCPlayerClient.SOC_PORT_DEFAULT.
     * <P>
     * 8880 is the default SOCPlayerClient port since jsettlers 1.0.4, per cvs
     * history.
     *
     * @since 1.1.09
     */
    ...
    /**
     * Property <tt>jsettlers.startrobots</tt> to start some robots when the server
     * starts. (The default is 0, no robots are started by default.)
     * <P>
     * 30% will be "smart" robots, the other 70% will be "fast" robots. Remember
     * that robots count against the {@link #PROP_JSETTLERS_CONNECTIONS max
     * connections} limit.
     *
     * @since 1.1.09
     */
     ...
     * games that a client can create at once. (The default is 5.) Once a game is
     * completed and deleted (all players leave), they can create another.
     *
     * @since 1.1.10
     */
     ...
     * chat channels that a client can create at once. (The default is 2.) Once a
     * channel is deleted (all members leave), they can create another.
     *
     * @since 1.1.10
     */
    ...
    /**
     * List and descriptions of all available JSettlers {@link Properties
     * properties}, such as {@link #PROP_JSETTLERS_PORT} and
     * {@link SOCDBHelper#PROP_JSETTLERS_DB_URL}.
     * <P>
     * Each property name is followed in the array by a brief description: [0] is a
     * property, [1] is its description, [2] is the next property, etc. (This was
     * added in 1.1.13 for {@link #printUsage(boolean)}}.
     *
     * @since 1.1.09
     */
    public static final String[] PROPS_LIST = { PROP_JSETTLERS_PORT, "TCP port number for server to bind to",
            PROP_JSETTLERS_CONNECTIONS, "Maximum connection count, including robots", PROP_JSETTLERS_STARTROBOTS,
            "Number of robots to create at startup", PROP_STAC_ROBOTS, "Number of Stac robots to create at startup",
            ...
            SOCDBHelper.PROP_JSETTLERS_DB_USER, "DB username", SOCDBHelper.PROP_JSETTLERS_DB_PASS, "DB password",
            SOCDBHelper.PROP_JSETTLERS_DB_URL, "DB connection URL", SOCDBHelper.PROP_JSETTLERS_DB_DRIVER,
    ...
    /**
     * Minimum required client version, to connect and play a game. Same format as
     * {@link soc.util.Version#versionNumber()}. Currently there is no enforced
     * minimum (0000).
     *
    ...
    /**
     * Minimum required client version, in "display" form, like "1.0.00". Currently
     * there is no minimum.
     *
     * @see #setClientVersSendGamesOrReject(StringConnection, int, boolean)
     */
     ...
     * {@link SOCGameListAtServer#createGame(String, String, Hashtable)}.
     *
    ...
    /**
     * Force robot to end their turn after this many seconds of inactivity. EDITED:
     * extended this to 30 sec from 8 sec as the robot needs to do some extra
     * processing for modelling beliefs
     *
    ...
    /**
     * Force robot to end their turn after this much inactivity, while they've made
     * a trade offer. EDITED: extended this to 3 min from 1 min to allow typing of
     * responses for human players
     *
     * @see #checkForExpiredTurns(long)
     * @since 1.1.11
     */
     ...
     * already truncates to 20 characters in SOCPlayerClient.getValidNickname.
     *
     * @see #createOrJoinGameIfUserOK(StringConnection, String, String, String,
     *      Hashtable)
     * @since 1.1.07
    ...
    /**
     * Maximum number of games that a client can create at the same time (default
     * 5). Once this limit is reached, the client must delete a game before creating
     * a new one. Set this to -1 to disable it; 0 will disallow any game creation.
     *
     * @since 1.1.10
     */
    ...
    /**
     * Maximum number of chat channels that a client can create at the same time
     * (default 2). Once this limit is reached, the client must delete a channel
     * before creating a new one. Set this to -1 to disable it; 0 will disallow any
    ...
    /**
     * For local simulation games (pipes, not TCP), the name of the pipe. Used to
     * distinguish simulation vs practice vs "real" games.
     *
     * @see soc.server.genericServer.LocalStringConnection
     */
    ...
    /**
     * Properties for the server, or empty if that constructor wasn't used. Property
     * names are held in PROP_* and SOCDBHelper.PROP_* constants.
     *
    ...
    /**
     * Robot default parameters; copied for each newly connecting robot. Changing
     * this will not change parameters of any robots already connected.
     *
     * @see #handleIMAROBOT(StringConnection, soc.message.SOCImARobot)
     * @see soc.robot.SOCRobotDM
     */
    public static SOCRobotParameters ROBOT_PARAMS_DEFAULT = new SOCRobotParameters(120, 35, 0.13f, 1.0f, 1.0f, 3.0f,
            1.0f, 1, 1);
     ...
     * @see soc.robot.SOCRobotDM
     */
    public static SOCRobotParameters ROBOT_PARAMS_SMARTER = new SOCRobotParameters(120, 35, 0.13f, 1.0f, 1.0f, 3.0f,
            1.0f, 0, 1);

    /**
     * Did the command line include --option / -o to set {@link SOCGameOption game
    ...
    /**
     * Status Message to send, nickname already logged into the system. Prepend to
     * {@link #MSG_NICKNAME_ALREADY_IN_USE}. The "take over" option is used for
     * reconnect when a client loses connection, and server doesn't realize it. A
     * new connection can "take over" the name after a minute's timeout.
     *
     * @since 1.1.08
     */
     ...
     * with a newer client version. Prepend to version number required. The "take
     * over" option is used for reconnect when a client loses connection, and server
     * doesn't realize it.
     *
    ...
    /**
     * Number of seconds before a connection is considered disconnected, and its
     * nickname can be "taken over" by a new connection from the same IP. Used when
     * no password is given by the new connection.
     *
     * @see #checkNickname(String, StringConnection)
     * @since 1.1.08
     */
    ...
    /**
     * Number of seconds before a connection is considered disconnected, and its
     * nickname can be "taken over" by a new connection from a different IP. Used
    ...
    /**
     * Flag determining whether the server should call the Toulouse parser when a
     * client sends a chat message; default: don't use the parser
     */
    ...
    /**
     * Create a Settlers of Catan server listening on TCP port p. You must start its
     * thread yourself.
     * <P>
     * In 1.1.07 and later, will also print game options to stderr if any option
     * defaults require a minimum client version, or if {@link #hasSetGameOptions}
     * is set.
     *
     * @param p                the TCP port that the server listens on
     * @param mc               the maximum number of connections allowed; remember
     *                         that robots count against this limit.
     * @param db
     * @param databaseUserName the user name for accessing the database
     * @param databasePassword the password for the user
     * @param useParser        flag specifying whether the server should call the
     *                         Toulouse parser
     */
     ...
     * thread yourself.
     * <P>
     * The database properties are {@link SOCDBHelper#PROP_JSETTLERS_DB_USER} and
     ...
     *
     * @param p     the TCP port that the server listens on
     * @param props null, or properties containing
     *              {@link #PROP_JSETTLERS_CONNECTIONS} and any other desired
     *              properties.
     * @since 1.1.09
     * @see #PROPS_LIST
     */
    public SOCServer(final int p, DBHelper db, Properties props) {
        ...
        CLIENT_MAX_CREATE_GAMES = init_getIntProperty(props, PROP_JSETTLERS_CLI_MAXCREATEGAMES,
                CLIENT_MAX_CREATE_GAMES);
        CLIENT_MAX_CREATE_CHANNELS = init_getIntProperty(props, PROP_JSETTLERS_CLI_MAXCREATECHANNELS,
    ...
    /**
     * Create a Settlers of Catan server listening on local stringport s. You must
     * start its thread yourself.
     * <P>
     * In 1.1.07 and later, will also print game options to stderr if any option
     * defaults require a minimum client version, or if {@link #hasSetGameOptions}
     * is set.
     *
     * @param s                the stringport that the server listens on. If this is
     *                         a "practice game" server on the user's local
     *                         computer, please use {@link #PRACTICE_STRINGPORT}.
     * @param mc               the maximum number of connections allowed; remember
     *                         that robots count against this limit.
     * @param databaseUserName the user name for accessing the database
     * @param databasePassword the password for the user
     ...
     *                         Toulouse parser
     */
    public SOCServer(String s, int mc, DBHelper db, String databaseUserName, String databasePassword,
            boolean useParser) {
     ...
     *                         Toulouse parser
     */
    public SOCServer(String s, int mc, DBHelper db, String databaseUserName, String databasePassword, SOCLogger logger,
            boolean useParser) {
        super(s);
        maxConnections = mc;
        this.db = db;
    ...
    /**
     * Common init for all constructors. Starts all server threads except the main
     * thread. If {@link #PROP_JSETTLERS_STARTROBOTS} is specified, those aren't
     * started until {@link #serverUp()}.
     *
     * @param databaseUserName Used for DB connect - not retained
     * @param databasePassword Used for DB connect - not retained
     * @param props            null, or properties containing
     *                         {@link #PROP_JSETTLERS_CONNECTIONS} and any other
     *                         desired properties. If <code>props</code> is null,
     *                         the properties will be created empty.
     * @param connectToDB      flag to allow creation of server without the database
     *                         for local practice games
     */
    ...
    /**
     * For initialization, get and parse an integer property, or use its default
     * instead.
     *
     ...
     * is bound and listening, in the main thread. If
     * {@link #PROP_JSETTLERS_STARTROBOTS} is specified, start those
     * {@link SOCRobotClient}s now.
     *
     * @since 1.1.09
     */
    public void serverUp() {
        /**
         * If we have any STARTROBOTS, start them up now. Each bot will have its own
         * thread and {@link SOCRobotClient}.
         */
        if ((props != null)
                && (props.containsKey(PROP_JSETTLERS_STARTROBOTS) || (props.containsKey(PROP_STAC_ROBOTS)))) {
            try {
                ...
                if (props.containsKey(PROP_JSETTLERS_STARTROBOTS))
                    ...
                    rcount = Integer.parseInt(props.getProperty(PROP_STAC_ROBOTS));// TODO: do we need the number of
                                                                                   // agents or just use config file for
                                                                                   // that?
                    stacRobots = true;
                }
    ...
    /**
     * Flag whether to use a special robot selection mechanism for the STAC system
     * evaluation experiment
     */
    public boolean useRobotSelectionForExperiment = false;

    /**
     * Reads the config.txt file and looks for the Agent fields and sets up the
     * robots accordingly
     */
            ...
            while (nextLine != null) {
                ...
                if (nextLine.contains("Agent")) {
                    ...
                    if (factory != null) {
                        ...
                        // TODO: this assumes the control parameters are always mentioned before the
                        // agents in the config file
                        ...
                        // try {
                        // Thread.sleep(1000);
                        // // Wait for these robots to have sent their SOCImARobot message to the server
                        // so that
                        // // they are entered into the SOCServer.robots array in the order in which
                        // they appear in the config.txt file.
                        // // This way the selection of 3 robots of the same type should work, because
                        // then they should be in consecutive
     ...
     *
     * WARNING: MUST HAVE THE channelList.takeMonitorForChannel(ch) before calling
     * this method
     *
     ...
     * <P>
     * After this, human players are free to join, until someone clicks "Start
     * Game". At that point, server will look for robots to fill empty seats.
     *
     * @param c      the Connection to be added; its name and version should already
     *               be set.
     * @param gaName the name of the game
     * @param gaOpts if creating a game with options, hashtable of
     *               {@link SOCGameOption}; otherwise null. Should already be
     *               validated, by calling
     *               {@link SOCGameOption#adjustOptionsToKnown(Hashtable, Hashtable)}.
     *
     * @return true if c was not a member of ch before, false if c was already in
     *         this game
     *
     * @throws SOCGameOptionVersionException if asking to create a game (gaOpts !=
     *                                       null), but client's version is too low
     *                                       to join because of a requested game
     *                                       option's minimum version in gaOpts.
     *                                       Calculated via
     *                                       {@link SOCGameOption#optionsNewerThanVersion(int, boolean, boolean, Hashtable)}.
     *                                       (this exception was added in 1.1.07)
     * @throws IllegalArgumentException      if client's version is too low to join
     *                                       for any other reason. (this exception
     *                                       was added in 1.1.06)
        ...
        if (gameExists) {
            ...
            if (gaOpts == null) {
                ...
                if (gVers > cliVers) {
                    ...
                    Vector optsValuesTooNew = SOCGameOption.optionsNewerThanVersion(cliVers, true, false, gaOpts);
            ...
            try {
                ...
                if ((strSocketName != null)
                        && (strSocketName.equals(PRACTICE_STRINGPORT) || strSocketName.equals(SIMULATION_STRINGPORT))) {
                ...
                if ((gVers <= cversMin) && (gaOpts == null)) {
                    ...
                    if (cversMin >= SOCNewGameWithOptions.VERSION_FOR_NEWGAMEWITHOPTIONS) {
                        ...
                        broadcast(SOCNewGameWithOptions.toCmd(gaName, gaOpts, gVers));
                    } else {
                        ...
                        if ((gaOpts != null) && (cversMax >= SOCNewGameWithOptions.VERSION_FOR_NEWGAMEWITHOPTIONS)) {
                            broadcastToVers(SOCNewGameWithOptions.toCmd(gaName, gaOpts, gVers),
                        ...
                        sb.append(gaName);
                        broadcastToVers(SOCNewGame.toCmd(sb.toString()), SOCGames.VERSION_FOR_UNJOINABLE, gVers - 1);
                    }
                }
     ...
     * {@link #forceEndGameTurn(SOCGame, String)}.
     * <P>
     ...
     *
     * @param c            the connection; if c is being dropped because of an
     *                     error, this method assumes that
     *                     {@link StringConnection#disconnect()} has already been
     *                     called. This method won't exclude c from any
     *                     communication about leaving the game, in case they are
    ...
    public boolean leaveGame(StringConnection c, String gm, boolean gameListLock) {
        ...
        for (playerNumber = 0; playerNumber < ga.maxPlayers; playerNumber++) {
            ...
            if ((player != null) && (player.getName() != null) && (player.getName().equals(plName))) {
                ...
                /**
                 * About to remove this player from the game. Before doing so: If a board-reset
                 * vote is in progress, they cannot vote once they have left. So to keep the
                 * game moving, fabricate their response: vote No.
                 */
            ...
            while (membersEnum.hasMoreElements()) {
                ...
                for (int pn = 0; pn < ga.maxPlayers; pn++) {
                    ...
                    if ((player != null) && (player.getName() != null)
                            && (player.getName().equals((String) member.getData()))) {
                        nameMatch = true;
                        break;
                    }
                }
        ...
        /**
         * if the leaving member was playing the game, and the game isn't over, then
         * decide: - Do we need to force-end the current turn? - Do we need to cancel
         * their initial settlement placement? - Should we replace the leaving player
         * with a robot?
         */
        if (isPlayer && (gameHasHumanPlayer || gameHasObserver)
                && ((ga.getPlayer(playerNumber).getPublicVP() > 0) || (gameState == SOCGame.START1A)
                        || (gameState == SOCGame.START1B))
                && (gameState < SOCGame.OVER) && !(gameState < SOCGame.START1A)) {
            ...
            if (ga.getPlayer(playerNumber).isRobot()) {
                ...
                /**
                 * get a robot to replace this human player; just in case, check game-version vs
                 * robots-version, like at new-game (readyGameAskRobotsJoin).
                 */
                foundNoRobots = false;
                ...
                if (robots.isEmpty()) {
                    ...
                    messageToGameWithMon(gm, new SOCGameTextMsg(gm, SERVERNAME,
                    ...
                    /**
                     * request a robot that isn't already playing this game or is not already
                     * requested to play in this game
                     */
                    boolean nameMatch = false;
                    StringConnection robotConn = null;

                    final int[] robotIndexes = robotShuffleForJoin(false); // Don't shuffle to distribute load (it's
                                                                           // leaving game why do we care?)
            ...
            /**
             * What to do if no robot was found to fill their spot? Must keep the game
             * going, might need to force-end current turn.
             */
            if (foundNoRobots) {
        ...
        /**
         * if the game has no players, or if they're all robots, then end the game and
     ...
     * <P>
     * Can be called for a player still in the game, or for a player who has left
     * ({@link SOCGame#removePlayer(String)} has been called).
     * <P>
     * <b>Locks:</b> Must not have ga.takeMonitor() when calling this method. May or
     * may not have <tt>gameList.takeMonitorForGame(ga)</tt>; use
     * <tt>hasMonitorFromGameList</tt> to indicate.
     * <P>
     * Not public, but package visibility, for use by {@link SOCGameTimeoutChecker}.
     *
     * @param ga                     The game to end turn
     * @param plNumber               player.getNumber; may or may not be current
     *                               player
     ...
     *                               {@link SOCGameList#takeMonitorForGame(String)
     *                               gameList.takeMonitorForGame(ga)}; if false,
     *                               this method will take this monitor at its
     *                               start, and release it before returning.
     * @return true if the turn was ended and game is still active; false if we find
     *         that all players have left and the gamestate has been changed here to
     *         {@link #OVER}.
     */
    boolean endGameTurnOrForce(SOCGame ga, final int plNumber, final String plName, StringConnection plConn,
        ...
        /**
         * Is a board-reset vote is in progress? If they're still a sitting player, to
         * keep the game moving, fabricate their response: vote No.
         */
        boolean gameVotingActiveDuringStart = false;
        ...
        if (plNumber == cpn) {
            /**
             * End their turn just to keep the game limping along. To prevent deadlock, we
             * must release gamelist's monitor for this game before calling endGameTurn.
             */

            if ((gameState == SOCGame.START1B) || (gameState == SOCGame.START2B)) {
                /**
                 * Leaving during 1st or 2nd initial road placement. Cancel the settlement they
                 * just placed, and send that cancel to the other players. Don't change
                 * gameState yet. Note that their 2nd settlement is removed in START2B, but not
                 * their 1st settlement. (This would impact the robots much more.)
                 */
            ...
            if (ga.canEndTurn(plNumber)) {
                ...
                /**
                 * Cannot easily end turn. Must back out something in progress. May or may not
                 * end turn; see javadocs of forceEndGameTurn and game.forceEndTurn. All start
                 * phases are covered here (START1A..START2B) because canEndTurn returns false
                 * in those gameStates.
                 */
                gameList.releaseMonitorForGame(gm);
                ga.takeMonitor();
                if (gameVotingActiveDuringStart) {
                    /**
                     * If anyone has requested a board-reset vote during game-start phases, we have
                     * to tell clients to cancel the vote request, because
                     * {@link soc.message.SOCTurn} isn't always sent during start phases. (Voting
                     * must end when the turn ends.)
                     */
            ...
            /**
             * Check if game is waiting for input from the player who is leaving, but who
             * isn't current player. To keep the game moving, fabricate their response. -
             * Board-reset voting: Handled above. - Waiting for discard: Handle here.
             */
            if ((gameState == SOCGame.WAITING_FOR_DISCARDS) && (ga.getPlayer(plNumber).getNeedToDiscard())) {
                /**
                 * For discard, tell the discarding player's client that they discarded the
                 * resources, tell everyone else that the player discarded unknown resources.
                 */
                gameList.releaseMonitorForGame(gm);
                ga.takeMonitor();
     ...
     *
     * @param noShuffle flag for not randomizing the robots order when running MCTS
     *                  simulations
    ...
    /**
     * Set up some robot opponents, running in our JVM for operator convenience. Set
     * up more than needed; when a game is started, game setup will randomize
     * whether its humans will play against smart or fast ones. (Some will be
     * SOCRobotDM.FAST_STRATEGY, some SMART_STRATEGY).
     * <P>
     * Before 1.1.09, this method was part of SOCPlayerClient.
     *
     * @param numFast  number of fast robots, with
     *                 {@link soc.robot.SOCRobotDM#FAST_STRATEGY FAST_STRATEGY}
     * @param numSmart number of smart robots, with
     *                 {@link soc.robot.SOCRobotDM#SMART_STRATEGY SMART_STRATEGY}
     * @return True if robots were set up, false if an exception occurred. This
     *         typically happens if a robot class, or SOCDisplaylessClient, can't be
     *         loaded, due to packaging of the server-only JAR.
    ...
    public boolean setupLocalRobots(final int numFast, final int numSmart) {
        try {
            ...
            for (int i = 0; i < numFast; ++i) {
                ...
                Thread t = SOCPlayerLocalRobotRunner.createAndStartRobotClientThread(factory, rname, strSocketName,
                        port, null);
     ...
     *
     * @param factory    the robot factory that creates the type of robots specified
    ...
    public boolean setupLocalRobots(SOCRobotFactory factory, String namePrefix, int numBots) {
        try {
             ...
             * System.out.println(">SOCServer.constructor ..."); String
             * numFeaturesAndActions = "160,120"; // TBD: retrieve these numbers
             * automatically deepAgent = new SimpleAgent(numFeaturesAndActions);
             * deepAgent.start();
             *
             * synchronized(deepAgent) { try{
             * System.out.println("Waiting for deepAgent to complete..."); deepAgent.wait();
             *
            ...
            for (int i = 0; i < numBots; ++i) {
                ...
                if (numBots > 1) {
                    // check whether the robot name contains a "|" - this separates multiple names
                    // for this robot type
    ...
    /**
     * Force this player (not current player) to discard, and report resources to
     * all players. Does not send gameState, which may have changed; see
     * {@link SOCGame#discardPickRandom(SOCResourceSet, int, SOCResourceSet, Random)}.
     * <P>
    ...
    private void forceGamePlayerDiscard(SOCGame cg, final int cpn, StringConnection c, String plName, final int pn) {
        ...
        if (!gp.fullyObservable) {
            ...
            messageToGameExcept(gaName, c,
                    new SOCPlayerElement(gaName, cpn, SOCPlayerElement.LOSE, SOCPlayerElement.UNKNOWN, totalRes), true);
        } else { // don't care if the player is connected as the below will send the message to
                 // all connected players
            reportRsrcGainLoss(gaName, discard, true, cpn, -1, null, null); // if perfect information game report exact
                                                                            // types/amounts to everyone
        }
    ...
    public void destroyGame(String gm, boolean haveMonitor) {
        ...
        if (cg != null) {
            ...
            /*
             * currentGameEventRecord.setSnapshot(cg); saveCurrentGameEventRecord(gm);
             * SOCGameRecord gr = (SOCGameRecord)gameRecords.get(gm); writeGameRecord(gm,
             * gr);
             */
     ...
     *
     * @param gm Game name
     * @return the game options (hashtable of {@link SOCGameOption}), or null if the
     *         game doesn't exist or has no options
     * @since 1.1.07
     */
    ...
    public Vector leaveAllChannels(StringConnection c) {
        if (c != null) {
            ...
            try {
                for (Enumeration k = channelList.getChannels(); k.hasMoreElements();) {
     ...
     * Send a message to the given channel
     *
     * WARNING: MUST HAVE THE gameList.takeMonitorForChannel(ch) before calling this
     * method
     *
     * @param ch  the name of the channel
     * @param mes the message to send
    ...
    public void messageToPlayer(StringConnection c, SOCMessage mes) {
        if ((c != null) && (mes != null)) {
            // currentGameEventRecord.addMessageOut(new SOCMessageRecord(mes, "SERVER",
            // c.getData()));
            c.put(mes.toCmd());
        }
    ...
    /**
     * Send a {@link SOCGameTextMsg} game text message to a player. Equivalent to:
     * messageToPlayer(conn, new {@link SOCGameTextMsg}(ga, {@link #SERVERNAME},
     * txt));
     *
     * @param c   the player connection
     * @param ga  game name
     ...
     * @param mes the message to send. If mes is a SOCGameTextMsg whose text begins
     *            with ">>>", the client should consider this an urgent message, and
     *            draw the user's attention in some way. (See
            ...
            if (v != null) {
                ...
                while (menum.hasMoreElements()) {
                    ...
                    if (c != null) {
                        // currentGameEventRecord.addMessageOut(new SOCMessageRecord(mes, "SERVER",
                        // c.getData()));
                        // System.err.println("Server - Message to game for host " + c.host() + ":" +
                        // mes.toCmd());
                        c.put(mes.toCmd());
                    }
                }
     ...
     * messageToGame(ga, new SOCGameTextMsg(ga, {@link #SERVERNAME}, txt));
     * <P>
     ...
     *
     * @param ga  the name of the game
     * @param txt the message text to send. If text begins with ">>>", the client
     *            should consider this an urgent message, and draw the user's
     *            attention in some way. (See
     *            {@link #messageToGameUrgent(String, String)})
     ...
     * <P>
     * <b>Locks:</b> MUST HAVE THE {@link SOCGameList#takeMonitorForGame(String)
    ...
    /**
     * Send an urgent SOCGameTextMsg to the given game. An "urgent" message is a
     * SOCGameTextMsg whose text begins with ">>>"; the client should draw the
     * user's attention in some way.
     * <P>
     * Like messageToGame, will take and release the game's monitor.
     *
     * @param ga  the name of the game
     * @param mes the message to send. If mes does not begin with ">>>", will
     ...
     * <P>
     * This method is called within a per-client thread, after connection is removed
     * from conns collection and version collection, and after c.disconnect() has
     * been called.
     *
     * @param c the connection
     */
    ...
    /**
     * Things to do when a new connection comes. If the connection is accepted, it's
     * added to {@link #unnamedConns} until the player "names" it by joining or
     * creating a game under their player name. Other communication is then done, in
     * {@link #newConnection2(StringConnection)}.
     * <P>
     * Also set client's "assumed version" to -1, until we have sent and received a
     * VERSION message.
     * <P>
     * This method is called within a per-client thread. You can send to client, but
     * can't yet receive messages from them.
     * <P>
        ...
        if (c != null) {
            ...
            try {
                if (this.connectionCount() >= maxConnections) {
                    SOCRejectConnection rcCommand = new SOCRejectConnection(
            ...
            try {
                /**
                 * prevent someone from connecting twice from the same machine (Commented out:
                 * This is a bad idea due to proxies, NAT, etc.)
                 */
                boolean hostMatch = false;
                /*
                 * Enumeration allConnections = this.getConnections();
                 *
                 * while(allConnections.hasMoreElements()) { StringConnection tempCon =
                 * (StringConnection)allConnections.nextElement(); if
                 * (!(c.host().equals("pippen")) && (tempCon.host().equals(c.host()))) {
                 * hostMatch = true; break; } }
                 */
                if (hostMatch) {
                    SOCRejectConnection rcCommand = new SOCRejectConnection(
                     ...
                     * {@link #newConnection2(StringConnection)} will try to wait for client
                     * version, and will send the list of channels and games.
                     */
                    c.setVersion(-1);
                    return true;
                }
     ...
     * ({@link SOCChannels}, {@link SOCGames})) when a new connection comes, part 2
     * - c has been accepted and added to a connection list. Unlike
     * {@link #newConnection1(StringConnection)}, no connection-list locks are held
    ...
    protected void newConnection2(StringConnection c) {
         ...
         * Games will be sent once we know the client's version, or have guessed that
         * it's too old (if the client doesn't tell us soon enough). So: Check if input
         * is waiting for us. If it turns out the waiting message is something other
         * than VERSION, server callback {@link #processFirstCommand} will set up the
         * version TimerTask using {@link SOCClientData#setVersionTimer}. The version
         * timer will call {@link #sendGameList} when it expires. If no input awaits us
         * right now, set up the timer here.
         */
     ...
     *
     * @param c           Connected client; its key data
     *                    ({@link StringConnection#getData()}) must not be null
     * @param isReplacing Are we replacing / taking over a current connection?
     * @throws IllegalArgumentException If c isn't already connected, if c.getData()
     *                                  returns null, or if nameConnection has
     *                                  previously been called for this connection.
     * @since 1.1.08
     */
    private void nameConnection(StringConnection c, boolean isReplacing) throws IllegalArgumentException {
     ...
     * connecting client. Or, send the set of changed games, if the client's guessed
     * version was wrong. The list includes a flag on games which can't be joined by
     * this client version ({@link SOCGames#MARKER_THIS_GAME_UNJOINABLE}).
     * <P>
     * If <b>entire list</b>, then depending on client's version, the message sent
     * will be either {@link SOCGames GAMES} or {@link SOCGamesWithOptions
     * GAMESWITHOPTIONS}. If <b>set of changed games</b>, sent as matching pairs of
     * {@link SOCDeleteGame DELETEGAME} and either {@link SOCNewGame NEWGAME} or
     * {@link SOCNewGameWithOptions NEWGAMEWITHOPTIONS}.
     * <P>
     * There are 2 possible scenarios for when this method is called:
     * <P>
     * - (A) Sending game list to client, for the first time: Iterate through all
     * games, looking for ones the client's version is capable of joining. If not
     * capable, mark the game name as such before sending it to the client. (As a
     * special case, very old client versions "can't know" about the game they can't
     * join, because they don't recognize the marker.) Also set the client data's
     * hasSentGameList flag.
     * <P>
     * - (B) The client didn't give its version, and was thus identified as an old
     * version. Now we know its newer true version, so we must tell it about games
     * that it can now join, which couldn't have been joined by the older assumed
     * version. So: Look for games with those criteria.
     * <P>
     ...
     * ({@link SOCGames#VERSION_FOR_UNJOINABLE}). Older clients won't be sent the
     * game names they can't join.
     * <P>
     * <b>Locks:</b> Calls {@link SOCGameListAtServer#takeMonitor()} /
     * releaseMonitor
     *
     * @param c        Client's connection; will call getVersion() on it
     * @param prevVers Previously assumed version of this client; if re-sending the
    ...
    public void sendGameList(StringConnection c, int prevVers) {
        ...
        gameList.takeMonitor();
        final boolean alreadySent = ((SOCClientData) c.getAppData()).hasSentGameList(); // Check while gamelist monitor
                                                                                        // is held
        ...
        /**
         * We release the monitor as soon as we can, even though we haven't yet sent the
         * list to the client. It's theoretically possible the client will get a NEWGAME
         * message, which is OK, or a DELETEGAME message, before it receives the list
         * we're building. NEWGAME is OK because the GAMES message won't clear the list
         * contents at client. DELETEGAME is less OK, but it's not very likely. If the
         * game is deleted, and then they see it in the list, trying to join that game
        ...
        /*
         * gaEnum = gameList.getGames(); int scores[] = new int[SOCGame.MAXPLAYERS];
         * boolean robots[] = new boolean[SOCGame.MAXPLAYERS]; while
         * (gaEnum.hasMoreElements()) { String gameName = (String)gaEnum.nextElement();
         * SOCGame theGame = gameList.getGameData(gameName); for (int i = 0; i <
         * SOCGame.MAXPLAYERS; i++) { SOCPlayer player = theGame.getPlayer(i); if
         * (player != null) { if (theGame.isSeatVacant(i)) { scores[i] = -1; robots[i] =
         * false; } else { scores[i] = player.getPublicVP(); robots[i] =
         * player.isRobot(); } } else { scores[i] = 0; } }
    ...
    /**
     * Check if a nickname is okay, and, if they're already logged in, whether a new
     * replacement connection can "take over" the existing one.
     * <P>
     ...
     * <P>
     * The "take over" option is used for reconnect when a client loses connection,
     * and server doesn't realize it. A new connection can "take over" the name
     * after a timeout; check the return value. (After
     * {@link #NICKNAME_TAKEOVER_SECONDS_SAME_IP} or
     * {@link #NICKNAME_TAKEOVER_SECONDS_DIFFERENT_IP} seconds) When taking over,
     * the new connection's client version must be able to join all games that the
     * old connection is playing, as returned by
     * {@link SOCGameListAtServer#playerGamesMinVersion(StringConnection)
    ...
    /**
     * For a nickname that seems to be in use, build a text message with the time
     * remaining before someone can attempt to take over that nickname. Used for
     * reconnect when a client loses connection, and server doesn't realize it. A
     * new connection can "take over" the name after a timeout.
     ...
     * @return message starting with "Please wait x seconds" or "Please wait x
     *         minute(s)"
     * @since 1.1.08
     */
    ...
    /**
     * For a nickname that seems to be in use, build a text message with the minimum
     * version number needed to take over that nickname. Used for reconnect when a
     * client loses connection, and server doesn't realize it. A new connection can
     * "take over" the name after a timeout.
     *
     * @param needsVersion Version number required to take it over; a positive
     *                     integer in the same format as
     *                     {@link SOCGame#getClientVersionMinRequired()}
    ...
    /**
     * Callback to process the client's first message command specially. Look for
     * VERSION message; if none is received, set up a timer to wait for version and
     * (if never received) send out the game list soon.
     *
    ...
    /**
     * ---MG Return the formatted date and time.
     *
    ...
    /**
     * ---MG Return the formatted date and time in a shorter format.
     *
     * @return String with the formatted date
     */
     ...
     * <P>
     * Note: When there is a choice, always use local information over information
     * from the message. For example, use the nickname from the connection to get
     * the player information rather than the player information from the message.
     * This makes it harder to send false messages making players do things they
     * didn't want to do.
     *
    ...
    public void processCommand(String s, StringConnection c) {
        ...
        if (messageType == SOCMessage.BCASTTEXTMSG || messageType == SOCMessage.TEXTMSG
                || messageType == SOCMessage.GAMETEXTMSG) {
            // String messageClassName = message.getClassNameShort(); //name of the message
            // class

            switch (messageType) {
                case SOCMessage.BCASTTEXTMSG:
                    SOCBCastTextMsg bCastTextMessage = (SOCBCastTextMsg) SOCMessage.toMsg(s);
                    // System.err.println("BROADCAST TEXT MESSAGE -- TEXT: " +
                    // bCastTextMessage.getText());
                    break;
                case SOCMessage.TEXTMSG:
                    SOCTextMsg textMessage = (SOCTextMsg) SOCMessage.toMsg(s);
                    // System.err.println("TEXT MESSAGE -- CHANNEL: " + textMessage.getChannel() +
                ...
                case SOCMessage.GAMETEXTMSG:
                    ...
                    // Note that in simulations, agents may attempt to send messages after the game
                    // has been completed, but before they have
                    ...
                    if (game != null) {
                        ...
                        String outputString;
                        // player is not initialised if the game has not started yet, so we'll have to
                        // generate a different message then
                        if (player == null) {
                            outputString = "player=" + sender + "|speaking-queue=" + game.speakingQueue + "|text="
                                    + text;
                        } else {
                            ...
                            // Enumeration<SOCPlayingPiece> e = (Enumeration<SOCPlayingPiece>)
                            // player.getCities().elements();
                            for (Enumeration<SOCPlayingPiece> e = (Enumeration<SOCPlayingPiece>) player.getCities()
                            ...
                            for (Enumeration<SOCPlayingPiece> e = (Enumeration<SOCPlayingPiece>) player.getSettlements()
                                    .elements(); e.hasMoreElements();)
                            ...
                            for (Enumeration<SOCPlayingPiece> e = (Enumeration<SOCPlayingPiece>) player.getRoads()
                                    .elements(); e.hasMoreElements();)
                            ...
                            outputString = formattedDate() + ":GAME-TEXT-MESSAGE:[game=" + gameTextMessage.getGame()
                                    + "|player=" + sender + "|speaking-queue=" + game.speakingQueue + "|"
                                    + resourcesString + "|knights=" + player.getNumKnights() + "|roads=["
                                    + roadCoordinates + "]|settlements=[" + stmtCoordinates + "]|cities=["
                                    + cityCoordinates + "]|dev-cards=" + player.getDevCards().getTotal() + "|text="
                                    + text + "]";
                        }
                        logString(game.getName(), outputString);

                        // we're not using speaking queues any more
                        // //now this player is done speaking and we can give the next player permission
                        // to speak
                        // game.currentSpeaker = null;
                        // giveNextPlayerPermissionToSpeak(game);
                    } else {
                        // NB: This is included as debug code to ensure the problem is correctly
                        // diagnosed and fixed. Should be removed once that's the case.
            ...
            if (mes != null) {
                switch (mes.getType()) {
                    ...
                    case SOCMessage.TEXTMSG:
                        ...
                        if (c.getData().equals("debug")) {
                            if (textMsgMes.getText().startsWith("*KILLCHANNEL*")) {
                                messageToChannel(textMsgMes.getChannel(), new SOCTextMsg(textMsgMes.getChannel(),
                                        SERVERNAME,
                    ...
                    case SOCMessage.JOINGAME:
                        ...
                        // currentGameEventRecord.setMessageIn(new SOCMessageRecord(mes, c.getData(),
                        // "SERVER"));
                    ...
                    /**
                     * Game option messages. For the best writeup of these messages' interaction
                     * with the client, see {@link soc.client.SOCPlayerClient.GameOptionServerSet}'s
                     * javadoc.
                     */
                    ...
                    /**
                     * ---MG a player sent a request to speak
                     */
                    ...
                    /**
                     * ---MG a player is starting a Register Trade interation
                     */
                    ...
                    /**
                     * ---MD Messages required for the save/load function
                     */
     ...
     */
    private static final String DEBUG_COMMANDS_HELP_RSRCS = "rsrcs: #cl #or #sh #wh #wo playername";

    /**
     * Used by {@link #DEBUG_COMMANDS_HELP}, etc.
     */
    private static final String DEBUG_COMMANDS_HELP_DEV = "dev: #typ playername";

    /**
     * Used by {@link #DEBUG_COMMANDS_HELP}, etc. Used with
     * {@link SOCGame#debugFreePlacement}.
     */
    private static final String DEBUG_CMD_FREEPLACEMENT = "*FREEPLACE*";

    /**
     * Used by {@link #processDebugCommand(StringConnection, String, String)}} when
     * *HELP* is requested.
     *
     * @since 1.1.07
     */
    public static final String[] DEBUG_COMMANDS_HELP = { "--- General Commands ---",
            "*ADDTIME*  add 30 minutes before game expiration", "*CHECKTIME*  print time remaining before expiration",
            "*VERSION*  show version and build information", "*WHO*   show players and observers of this game",
            "--- Debug Commands ---", "*BCAST*  broadcast msg to all games/channels",
            "*GC*    trigger the java garbage-collect", "*KILLBOT*  botname  End a bot's connection",
            "*KILLGAME*  end the current game", DEBUG_CMD_FREEPLACEMENT + " 1 or 0  Start or end 'Free Placement' mode",
            "*RESETBOT* botname  End a bot's connection", "*STATS*   server stats and current-game stats",
            "*STOP*  kill the server", "--- Debug Resources ---", DEBUG_COMMANDS_HELP_RSRCS,
            "Example  rsrcs: 0 3 0 2 0 Myname", DEBUG_COMMANDS_HELP_DEV, "Example  dev: 2 Myname",
            "Development card types are:", // see SOCDevCardConstants
            "0 robber", "1 road-building", "2 year of plenty", "3 monopoly", "4 governors house", "5 market",
            "6 university", "7 temple", "8 chapel" };
    ...
    public void processDebugCommand(StringConnection debugCli, String ga, String dcmd) {
        ...
        if (dcmdU.startsWith("*KILLGAME*")) {
            messageToGameUrgent(ga,
                    ">>> ********** " + (String) debugCli.getData() + " KILLED THE GAME!!! ********** <<<");
            gameList.takeMonitor();
    ...
    /**
     * The server is being cleanly stopped. Shut down with a final message "The game
    ...
    /**
     * The server is being cleanly stopped. Send a final message, disconnect all the
     * connections, disconnect from database if connected. Currently called only by
     * the debug command "*STOP*", and by SOCPlayerClient's locally hosted TCP
     * server.
     *
     * @param stopMsg Final text message to send to all connected clients, or null.
     *                Will be sent as a {@link SOCBCastTextMsg}. As always, if
     *                message starts with ">>" it will be considered urgent.
     */
    public synchronized void stopServer(String stopMsg) {
        if (stopMsg != null) {
            try {
                ...
                /*
                 * due to the order of things getting closed, this might trow a nullpointer
                 * exception; don't care as its only a text message, but we want the below logic
                 * to get executed
                 */}
        }
    ...
    /**
     * authenticate the user: see if the user is in the db, if so then check the
     * password. if they're not in the db, but they supplied a password, then send a
     * message (not OK). if they're not in the db, and no password, then ok.
     *
    ...
    private boolean authenticateUser(StringConnection c, String userName, String password) {
        ...
        try {
            ...
            if (sqle.getCause().getClass() == EOFException.class) {
                ...
                try {
                    ...
                    c.put(SOCStatusMessage.toCmd(SOCStatusMessage.SV_PROBLEM_WITH_DB, c.getVersion(),
        ...
        if (userPassword != null) {
            if (!userPassword.equals(password)) {
                c.put(SOCStatusMessage.toCmd(SOCStatusMessage.SV_PW_WRONG, c.getVersion(),
            ...
            c.put(SOCStatusMessage.toCmd(SOCStatusMessage.SV_NAME_NOT_FOUND, c.getVersion(),
        ...
        try {
            ...
            // if (params != null)
            // D.ebugPrintln("*** Robot Parameters for " + mes.getNickname() + " = " +
            // params);
    ...
    /**
     * Handle the "version" message, client's version report. May ask to disconnect,
     * if version is too old. Otherwise send the game list. If we've already sent
     * the game list, send changes based on true version. If they send another
     * VERSION later, with a different version, disconnect the client.
     * <P>
     * Along with the game list, the client will need to know the game option info.
     * This is sent when the client asks (after VERSION) for
     * {@link SOCGameOptionGetInfos GAMEOPTIONGETINFOS}.
     *
     ...
     * {@link SOCRejectConnection REJECTCONNECTION}. If we haven't yet sent the game
     * list, send now. If we've already sent the game list, send changes based on
     * true version.
     * <P>
     * Along with the game list, the client will need to know the game option info.
     * This is sent when the client asks (after VERSION) for
     * {@link SOCGameOptionGetInfos GAMEOPTIONGETINFOS}. Game options are sent after
     * client version is known, so the list of sent options is based on client
     * version.
     * <P>
     ...
     * {@link SOCClientData.SOCCDCliVersionTask#run()}.
     *
    ...
    /**
     * Handle the "join a channel" message. If client hasn't yet sent its version,
     * assume is version 1.0.00 ({@link #CLI_VERSION_ASSUMED_GUESS}), disconnect if
     * too low.
     *
     * @param c   the connection that sent the message
     * @param mes the messsage
        ...
        if (c != null) {
            ...
            if (c.getData() == null) {
                if (msgUser.length() > PLAYER_NAME_MAX_LENGTH) {
                    c.put(SOCStatusMessage.toCmd(SOCStatusMessage.SV_NEWGAME_NAME_TOO_LONG, cliVers,
                ...
                if (nameTimeout == -1) {
                    ...
                    c.put(SOCStatusMessage.toCmd(SOCStatusMessage.SV_NAME_IN_USE, cliVers,
            ...
            /*
             * if (!checkChannelName(mes.getChannel())) { return; }
             */
            final String ch = mes.getChannel().trim();
            if (!SOCMessage.isSingleLineAndSafe(ch)) {
                c.put(SOCStatusMessage.toCmd(SOCStatusMessage.SV_NEWGAME_NAME_REJECTED, cliVers,
            ...
            /**
             * Now that everything's validated, name this connection/user/player. If
             * isTakingOver, also copies their current game/channel count.
             */
            if (c.getData() == null) {
                c.setData(msgUser);
             ...
             */
            if ((!channelList.isChannel(ch)) && (CLIENT_MAX_CREATE_CHANNELS >= 0)
                    && (CLIENT_MAX_CREATE_CHANNELS <= ((SOCClientData) c.getAppData()).getcurrentCreatedChannels())) {
                c.put(SOCStatusMessage.toCmd(SOCStatusMessage.SV_NEWCHANNEL_TOO_MANY_CREATED, cliVers,
                        SOCStatusMessage.MSG_SV_NEWCHANNEL_TOO_MANY_CREATED
                                + Integer.toString(CLIENT_MAX_CREATE_CHANNELS)));
            ...
            c.put(SOCStatusMessage.toCmd(SOCStatusMessage.SV_OK, "Welcome to STACSettlers!"));
    ...
    /**
     * Handle the "I'm a robot" message. Robots send their {@link SOCVersion} before
     * sending this message. Their version is checked here, must equal server's
     * version.
     *
     * @param c   the connection that sent the message
     * @param mes the messsage
    ...
    private void handleIMAROBOT(StringConnection c, SOCImARobot mes) {
        if (c != null) {
            ...
            final boolean isBuiltIn = (rbc == null) || (rbc.equals(SOCImARobot.RBCLASS_BUILTIN));
            if (isBuiltIn) {
                if (cliVers != srvVers) {
                    ...
                    c.put(new SOCRejectConnection(rejectMsg).toCmd());
                    c.disconnectSoft();
                    D.ebugPrintlnINFO(formattedDate() + ":" + "Rejected robot " + mes.getNickname() + ": Version "
                            + cliVers + " does not match server version");
                    return; // <--- Early return: Robot client too old ---
                } else {
                    D.ebugPrintlnINFO(
                            formattedDate() + ":" + "Robot arrived: " + mes.getNickname() + ": built-in type");
                }
            } else {
            ...
            if ((c.getData() == null) && (0 != checkNickname(mes.getNickname(), c, false))) {
                c.put(SOCStatusMessage.toCmd(SOCStatusMessage.SV_NAME_IN_USE, cliVers, MSG_NICKNAME_ALREADY_IN_USE));
                SOCRejectConnection rcCommand = new SOCRejectConnection(MSG_NICKNAME_ALREADY_IN_USE);
                c.put(rcCommand.toCmd());
                System.err.println(
                        formattedDate() + ":" + "Robot login attempt, name already in use: " + mes.getNickname());
                // c.disconnect();
                c.disconnectSoft();
            ...
            // In case of disconnect-reconnect, don't print the error or re-arrival debug
            // announcements.
            // The robot's nickname is used as the key for the disconnect announcement.
            {
                ...
                if (depart != null) {
                    ...
                    ConnExcepDelayedPrintTask arrive = (ConnExcepDelayedPrintTask) cliConnDisconPrintsPending.get(c);
                    if (arrive != null) {
                        arrive.cancel();
            ...
            try {
                ...
                System.err
                        .println(formattedDate() + ":" + "Error retrieving robot parameters from db: Using defaults.");
            }
            ...
            robots.addElement(c);
            // for our specialist experimental setup, sort the robots alphabetically by name
            // so that the server picks 3 robots of the saem type later
            if (useRobotSelectionForExperiment) {
    ...
    /**
     * Keep track of whether the last trade offer was forced for logging purposes.
     * (So we can say whether a forced offer was accepted or not.)
     */
    private boolean lastTradeOfferWasForceAccept = false;

    /**
     * Equivalent use to {@link lastTradeOfferWasForceAccept} Used to maintain the
     * previous persuasion move
     */
    ...
    /**
     * The pending trade for each game to keep track of the confirmation panel. The
     * stored HashMap for each game contains: "players" (ListArray of String),
     * "offeringPlayer" (int), "acceptingPlayer" (int)
     */
    private HashMap<String, HashMap> pendingTrades;

    /**
     * Handle game text messages, including debug commands. Also handles the
     * trade/negotiations messages sent via the chat interface and executes the
     * trade if it was agreed. Was part of processCommand before 1.1.07.
     *
     * @since 1.1.07
     * @param c              the connection to the client that sent the message
    ...
    private void handleGAMETEXTMSG(StringConnection c, SOCGameTextMsg gameTextMsgMes) {
        ...
        if (cmdTxtUC.startsWith("*ADDTIME*") || cmdTxtUC.startsWith("ADDTIME")) {
            ...
            if (ga.isPractice) {
                ...
                messageToGameUrgent(gaName, ">>> This game will expire in "
                        + ((ga.getExpiration() - System.currentTimeMillis()) / 60000) + " minutes.");
            }
        }
        ...
        if (cmdTxtUC.startsWith("*CHECKTIME*")) {
            ...
            messageToPlayer(c, gaName, "Java Settlers Server " + Version.versionNumber() + " (" + Version.version()
                    + ") build " + Version.buildnum());
        } else if (cmdTxtUC.startsWith("*STATS*")) {
            final long diff = System.currentTimeMillis() - startTime;
            final long hours = diff / (60 * 60 * 1000), minutes = (diff - (hours * 60 * 60 * 1000)) / (60 * 1000),
            ...
            messageToPlayer(c, gaName, "> Version: " + Version.versionNumber() + " (" + Version.version() + ") build "
                    + Version.buildnum());
        ...
        else // if (c.getData().equals("debug") || (c instanceof LocalStringConnection))
        {
            if (cmdTxtUC.startsWith("RSRCS:")) {
            ...
            } else if (StacRobotBrain.isChatNegotiation() && (cmdText.startsWith(StacTradeMessage.TRADE)
                    || cmdText.startsWith(StacRobotDialogueManager.JMT_TRADE))) {
                ...
                // System.err.println("Server - Handling trade message from host " + c.host() +
                // " :" + cmdText);
                // chop off the prefix and try to parse the message as a trade offer- if that
                // works, log it
                // {
                // SOCPlayer player = ga.getPlayer((String) c.getData());
                // System.err.println(ga.getTurnCount() + " * " + player.getName() + "(" +
                // player.getPlayerNumber() + ") - Trade event message: " + cmdText);
                // }

                // Note who the recipient of the message is - in the case of rej/acc/nr, this
                // will indicate what trade they are responding to
                ...
                if (tr.getOffer() != null) {
                    ...
                    SOCPlayer player = ga.getPlayer((String) c.getData());
                    // add this offer to the current offer so we will know not to end the current
                    // player's turn too quickly
                    ...
                    if (!usingPersuasion) {
                        ...
                        db.logChatTradeOffer(player, tr, ga.getCurrentPlayerNumber(), false, tr.getPersuasiveMove(),
                                player.getGame().getRoundCount());
                    }
                    lastTradeOfferWasForceAccept = tr.isForced();
                    lastTradeOfferPersuasion = tr.getPersuasiveMove();

                    // add it to the corresponding position in the tradeResponse array for tracking
                    // purposes
                    ...
                    if (player.getPlayerNumber() == ga.getCurrentPlayerNumber()) {
                        // newProt //treat counter-offers from the current player as new offers so we
                        // can clear the responses from the previous offer
                        // System.err.println("--- Server: clearing trade response for all players
                        // (1)");
                        // tradeResponses.put(ga.getName(),new StacTradeMessage[ga.maxPlayers]);
                        StacTradeMessage[] responses = tradeResponses.get(ga.getName());
                        responses[player.getPlayerNumber()] = new StacTradeMessage(sender, receivers, offer,
                                tr.isForced(), lastTradeOfferPersuasion, tr.getNLChatString());
                        D.ebugPrintlnINFO("--- Server: Updated trade response (1); Player " + player.getName()
                                + tradeResponsesString(gaName));
                    } else if (offer.getTo()[ga.getCurrentPlayerNumber()]) {
                        StacTradeMessage[] responses = tradeResponses.get(ga.getName());
                        responses[player.getPlayerNumber()] = new StacTradeMessage(sender, receivers, offer,
                                tr.isForced(), lastTradeOfferPersuasion, tr.getNLChatString());
                        D.ebugPrintlnINFO("--- Server: Updated trade response (2); Player " + player.getName()
                                + tradeResponsesString(gaName));
                    } else {
                    ...
                    if (responses[to] == null) {
                        if (tr.isAccept()) { // accepts shouldn't be missed so print out if this happens;
                            D.ebugERROR("Cannot accept a null trade message \n msg: " + tr.toString() + " from "
                                    + player.getPlayerNumber());
                            messageToGameWithMon(gaName, new SOCGameTextMsg(gaName, SERVERNAME,
                                    "*** " + player.getName() + " sent an invalid ACC message."));
                            return; // don't forward an invalid accept to the other players, this will mess up the
                                    // internal representation of the robots
                        }
                        // rejects and no-responses are fine here, but send them to the participants to
                        // avoid robots getting stuck waiting for answers
                        messageToGame(gaName, new SOCGameTextMsg(gaName, (String) c.getData(), cmdText));
                        return;
                    }

                    // link the response to the original offer(NOTE: the offer and the response are
                    // in the same StacTradeMessage object, despite not coming from the same player)
                    if (to != ga.getCurrentPlayerNumber() && player.getPlayerNumber() != ga.getCurrentPlayerNumber()) {
                        messageToGame(gaName, new SOCGameTextMsg(gaName, (String) c.getData(), cmdText));
                        return;
                        // ignore responses to other players but the current player as these are the
                        // rules of the game. send the message to the game as one of the players may be
                        // waiting for it;
                    } else if (player.getPlayerNumber() == ga.getCurrentPlayerNumber() && tr.isReject()) {
                        // ignore rejects from the current player as this will interfere with the logic
                        // below, but again send them to the game
                        messageToGame(gaName, new SOCGameTextMsg(gaName, (String) c.getData(), cmdText));
                        return;
                    } else {
                        // it can be a reject/acc or no-response to current player then check who is
                        // receiving this answer and combine with the offer in a new trade message
                        ...
                        if (tr.isAccept()) {
                            StacTradeMessage res = new StacTradeMessage(tr.getSender(), tr.getReceivers(),
                                    tr.isAccept(), tr.isReject(), tr.isNoResponse(), responses[to].getOffer(),
                                    responses[to].isForced(), tr.getNLChatString());
                            ...
                            D.ebugPrintlnINFO("--- Server: NOT updating trade response (3); Player " + player.getName()
                                    + "\n  - old message: " + tr.toString() + tradeResponsesString(gaName));
                        }
                    }
                    ...
                    if (tr.isAccept()) {
                        if (responses[to].isAccept()) {
                            ...
                            if (responses[to].getOffer().getFrom() == player.getPlayerNumber()) {
                                // newProt tradeResponses.put(ga.getName(),new
                                // StacTradeMessage[ga.maxPlayers]);//clear the trade responses after a trade
                                // was accepted
                                // System.err.println("--- Server: clearing trade response for player " + to +
                                // tradeResponsesString(gaName));
                                // responses[to] = null;
                                D.ebugPrintlnINFO("--- Server: NOT clearing trade response for player " + to
                                        + " immediately before executing the trade" + tradeResponsesString(gaName));

                                // all preconditions for executing a trade are met
                                // but we need a final confirmation from the human players involved in this
                                // trade that they wants to make it
                                final int offeringNumber = player.getPlayerNumber();
                                final int acceptingNumber = to;
                                if (!ga.getPlayer(offeringNumber).isRobot()
                                        || !ga.getPlayer(acceptingNumber).isRobot()) {
                                    ...
                                    SOCTradeOffer off = ga.getPlayer(offeringNumber).getCurrentOffer(); // this offer
                                                                                                        // will be
                                                                                                        // exeuted in
                                                                                                        // the end in
                                                                                                        // SOCGame.makeTrade(...)
                                    ...
                                    StacConfirmTradeRequest confirmationRequest = new StacConfirmTradeRequest(gaName,
                                            p1, p2);
                                    messageToGame(gaName, confirmationRequest);
                                    return;
                                }

                                // messageToGame(gaName, new SOCGameTextMsg(gaName, (String) c.getData(),
                                // cmdText)); //we don't need to tell everyone that another accept was sent as
                                // they can see from the trade execution
                                // The second accepter is actually the offerer, because it is the initiator of
                                // the original offer
                                ...
                                if (COLLECT_VALUE_FUNCTION_APPROX)
                                    messageToGame(ga.getName(),
                                            new SOCCollectData(ga.getName(), ga.getCurrentPlayerNumber()));
                                return;
                            } else {
                                // this shouldn't happen in the only robots case and for a human player this is
                                // restricted on the client side
                    ...
                    // then check if everyone has responded(by making sure there are no null
                    // responses unless its a human player)
                    boolean everyoneResponded = true;
                    for (int i = 0; i < ga.maxPlayers; i++) {
                        if (responses[i] == null) {// player didn't reply
                            // need to handle the human special case of not responding to messages that are
                            // not intended for him
                            SOCPlayer pl = ga.getPlayer(i);
                            if (!pl.isRobot()) {
                                // we have to loop over all the offers and check if this player is a recipient
                                // of any offers
                                for (StacTradeMessage trd : responses) {
                                    if (trd != null) {
                                        if (trd.getOffer().getTo()[i] && !trd.isAccept() && !trd.isReject()
                                                && !trd.isNoResponse()) {
                                            everyoneResponded = false;// the human player is a recipient of an offer so
                                                                      // there should have been a response from him
                                        }
                                    }
                                }
                            } else
                    ...
                    if (everyoneResponded) {
                        ...
                        for (int i = 0; i < ga.maxPlayers; i++) {
                            if (responses[i] == null) {
                                // ignore the no responses from human players, just handle them to avoid a
                                // nullpointer
                            } else {
                                if (responses[i].isAccept()) {
                                    ...
                                    D.ebugERROR(
                                            " received a trade message which is not an offer/acc/rej/block/block comply");
                            }
                        }

                        // make sure no unusual scenarios are encountered and also handle the nobody
                        // accepted case.
                        if (nAcc == 1 && nOff == 1) {
                            ...
                            if (!cpResp.isNoResponse() && !cpResp.isReject()) {
                                ...
                                // the current player rejected the offer //this doesn't happen anymore because
                                // we clean responses every time the current player sends a new offer
                            } else {
                                // smth went wrong report an error
                                D.ebugERROR(" the current player was not involved in the trade; clear responses");
                            }
                            D.ebugPrintlnINFO("--- Server: clearing trade response for all players (2)"
                                    + tradeResponsesString(gaName));
                            tradeResponses.put(ga.getName(), new StacTradeMessage[ga.maxPlayers]);// as this is a reject
                                                                                                  // clear the responses
                        } else if (nOff > 1 && nAcc == 0) {
                            // waiting for the players to decide on a trade
                        } else if ((nAcc == 1 || nAcc > 1) && (nOff > 1 || nOff == 1)) { // the 1:1 case is covered in
                                                                                         // the first if clause
                            // waiting for players to decide on a trade
                        } else if (nAcc > 1 && nOff == 0) {
                            D.ebugERROR("The offer should have been executed, we shouldn't be here");// this can't
                                                                                                     // really happen
                                                                                                     // but report it
                                                                                                     // just in case
                        } else
    ...
    /**
     * Print time-remaining and other game stats. Includes more detail beyond the
     * end-game stats sent in {@link #sendGameStateOVER(SOCGame)}.
     *
    ...
    private void processDebugCommand_checktime(StringConnection c, final String gaName, SOCGame gameData) {
        ...
        if (!gameData.isPractice) // practice games don't expire
        {
            String expireMsg = ">>> This game will expire in "
                    + ((gameData.getExpiration() - System.currentTimeMillis()) / 60000) + " minutes.";
            messageToPlayer(c, gaName, expireMsg);
        }
    }

    /**
     * Process the <tt>*FREEPLACE*</tt> Free Placement debug command. Can turn it
     * off at any time, but can only turn it on during your own turn after rolling
     * (during game state {@link SOCGame#PLAY1}).
     *
     * @param c      Connection (client) sending this message
     * @param gaName Game to which this applies
     * @param arg    1 or 0, to turn on or off, or empty string or null to print
     *               current value
     * @since 1.1.12
     */
    private final void processDebugCommand_freePlace(StringConnection c, final String gaName, final String arg) {
        SOCGame ga = gameList.getGameData(gaName);
        if (ga == null)
            return;
        ...
        if (ppValue != ppWanted) {
            if (!ppWanted) {
                try {
                    ...
                    if (wasInitial) {
                        messageToPlayer(c, gaName, "* To exit this debug mode, all players must have either");
                        messageToPlayer(c, gaName, "  1 settlement and 1 road, or all must have at least 2 of each.");
                    } else {
                        messageToPlayer(c, gaName, "* Could not exit this debug mode: " + e.getMessage());
                    }
                    return; // <--- early return ---
                }
            } else {
                if (c.getVersion() < SOCDebugFreePlace.VERSION_FOR_DEBUGFREEPLACE) {
                    messageToPlayer(c, gaName, "* Requires client version "
                            + Version.version(SOCDebugFreePlace.VERSION_FOR_DEBUGFREEPLACE) + " or newer.");
                    return; // <--- early return ---
                }
                ...
                if (ga.getCurrentPlayerNumber() != cliPl.getPlayerNumber()) {
                    messageToPlayer(c, gaName, "* Can do this only on your own turn.");
                    return; // <--- early return ---
                }
                if ((ga.getGameState() != SOCGame.PLAY1) && !ga.isInitialPlacement()) {
                    messageToPlayer(c, gaName, "* Can do this only after rolling the dice.");
                    return; // <--- early return ---
                }
        ...
        messageToPlayer(c, gaName, "- Free Placement mode is " + (ppWanted ? "ON -" : "off -"));
    ...
    /**
     * Handle the "join a game" message: Join or create a game. Will join the game,
     * or return a STATUSMESSAGE if nickname is not OK. Clients can join game as an
     * observer, if they don't SITDOWN after joining.
     * <P>
     * If client hasn't yet sent its version, assume is version 1.0.00
     * ({@link #CLI_VERSION_ASSUMED_GUESS}), disconnect if too low. If the client is
     * too old to join a specific game, return a STATUSMESSAGE. (since 1.1.06)
     *
     * @param c   the connection that sent the message
     * @param mes the messsage
     */
    private void handleJOINGAME(StringConnection c, SOCJoinGame mes) {
        if (c != null) {
            ...
            createOrJoinGameIfUserOK(c, mes.getNickname().trim(), mes.getPassword(), mes.getGame().trim(), null);

        }
    }

    /**
     * ---MG get the value from the database whether the user will be shown the
     * trade reminder dialog
     *
    ...
    boolean showDialogForTradeReminderToUser(String user) {
        ...
        // System.err.println("SOCServer: Could not retrieve value for Show Dialog from
        // database");
        // }
        //
    ...
    /**
     * Update the flag in the database whether the user must play a training game
     * next.
     *
     * @param user the user for which to query the database
     ...
     * NEWGAMEWITHOPTIONSREQUEST may be the first message with the client's username
     * and password, so c.getData() may be null. Assumes client's version is already
     * received or guessed.
     * <P>
     * Game name and player name have a maximum length and some disallowed
     * characters; see parameters. Check the client's
     * {@link SOCClientData#getCurrentCreatedGames()} vs
     * {@link #CLIENT_MAX_CREATE_GAMES}.
     * <P>
     * If client is replacing/taking over their own lost connection, first tell them
     * they're rejoining all their other games. That way, the requested game's
     * window will appear last, not hidden behind the others.
     * <P>
     * <b>Process if gameOpts != null:</b>
     * <UL>
     * <LI>if game with this name already exists, respond with
     * STATUSMESSAGE({@link SOCStatusMessage#SV_NEWGAME_ALREADY_EXISTS
     ...
     * STATUSMESSAGE({@link SOCStatusMessage#SV_NEWGAME_OPTION_UNKNOWN
     * SV_NEWGAME_OPTION_UNKNOWN}) <br>
     * - if any are too new for client's version, resp with
     * STATUSMESSAGE({@link SOCStatusMessage#SV_NEWGAME_OPTION_VALUE_TOONEW
     ...
     * {@link #joinGame(SOCGame, StringConnection, boolean, boolean)}
     * <LI>send game status details to requesting client, via
     * {@link #joinGame(SOCGame, StringConnection, boolean, boolean)}
     * </UL>
     *
     * @param c        connection requesting the game, must not be null
     * @param msgUser  username of client in message. Must pass
     *                 {@link SOCMessage#isSingleLineAndSafe(String)} and be at most
     *                 {@link #PLAYER_NAME_MAX_LENGTH} characters.
     * @param msgPass  password of client in message
     * @param gameName name of game to create/join. Must pass
     *                 {@link SOCMessage#isSingleLineAndSafe(String)} and be at most
     *                 {@link #GAME_NAME_MAX_LENGTH} characters.
     * @param gameOpts if game has options, contains {@link SOCGameOption} to create
     *                 new game; if not null, will not join an existing game. Will
     *                 validate by calling
     *                 {@link SOCGameOption#adjustOptionsToKnown(Hashtable, Hashtable)}.
     *
     * @since 1.1.07
     */
    private void createOrJoinGameIfUserOK(StringConnection c, final String msgUser, String msgPass,
            final String gameName, Hashtable gameOpts) {
        if (msgPass != null)
            msgPass = msgPass.trim();
         ...
         */
        if ((!gameList.isGame(gameName)) && (CLIENT_MAX_CREATE_GAMES >= 0)
                && (CLIENT_MAX_CREATE_GAMES <= ((SOCClientData) c.getAppData()).getCurrentCreatedGames())) {
            c.put(SOCStatusMessage.toCmd(SOCStatusMessage.SV_NEWGAME_TOO_MANY_CREATED, cliVers,
        ...
        /**
         * If we have game options, we're being asked to create a new game. Validate
         * them and ensure the game doesn't already exist.
         */
        if (gameOpts != null) {
            if (gameList.isGame(gameName)) {
                c.put(SOCStatusMessage.toCmd(SOCStatusMessage.SV_NEWGAME_ALREADY_EXISTS, cliVers,
            ...
            if (!SOCGameOption.adjustOptionsToKnown(gameOpts, null)) {
                c.put(SOCStatusMessage.toCmd(SOCStatusMessage.SV_NEWGAME_OPTION_UNKNOWN, cliVers,
        ...
        /**
         * Try to create or add player to game, and tell the client that everything is
         * ready; if game doesn't yet exist, it's created in connectToGame, and
         * announced there to all clients.
         * <P>
         * If client's version is too low (based on game options, etc), connectToGame
         * will throw an exception; tell the client if that happens.
         * <P>
        ...
        try {
            if (isTakingOver) {
                /**
                 * Rejoin the requested game. First, rejoin all other games of this client. That
                 * way, the requested game's window will appear last, not hidden behind the
                 * others. For each game, calls joinGame to send JOINGAMEAUTH and the entire
                 * state of the game to client.
                 */
            ...
            {
                /**
                 * send JOINGAMEAUTH to client, send the entire state of the game to client,
            ...
            c.put(SOCStatusMessage.toCmd(SOCStatusMessage.SV_NEWGAME_OPTION_VALUE_TOONEW, cliVers,
                    "Cannot create game with these options; requires version " + Integer.toString(e.gameOptsVersion)
                            + SOCMessage.sep2_char + gameName + SOCMessage.sep2_char + e.problemOptionsList()));
        } catch (IllegalArgumentException e) {
            // Let them know they can't join; include the game's version.

            c.put(SOCStatusMessage.toCmd(SOCStatusMessage.SV_CANT_JOIN_GAME_VERSION, cliVers,
    ...
    private void handleLEAVEGAME_member(StringConnection c, final String gaName) {
        ...
        if (gameDestroyed) {
            ...
            /*
             * SOCLeaveGame leaveMessage = new SOCLeaveGame((String)c.getData(), c.host(),
             * mes.getGame()); messageToGame(mes.getGame(), leaveMessage);
     ...
     * {@link #handleLEAVEGAME(StringConnection, SOCLeaveGame)}. Ignore the robot
    ...
    private void handleSITDOWN(StringConnection c, SOCSitDown mes) {
        if (c != null) {
            ...
            if (ga != null) {
                ...
                /*
                 * for (int i = 0; i < SOCGame.MAXPLAYERS; i++) { if (ga.getPlayer(i).getName()
                 * == (String)c.getData()) { canSit = false; break; } }
                 */
                // D.ebugPrintln("ga.isSeatVacant(mes.getPlayerNumber()) =
                // "+ga.isSeatVacant(mes.getPlayerNumber()));
                /**
                 * make sure a person isn't sitting here already; if a robot is sitting there,
                 * dismiss the robot.
                 */
                ga.takeMonitor();
                ...
                try {
                    if (ga.isSeatVacant(mes.getPlayerNumber())) {
                        ...
                        if (seatedPlayer.isRobot() && (!ga.isSeatLocked(mes.getPlayerNumber()))
                            ...
                            /**
                             * this connection has to wait for the robot to leave and then it can sit down
                             */
                ...
                if (canSit) {
                    ...
                    /**
                     * if the robot can't sit, tell it to go away. otherwise if game is full, tell
                     * the player.
                     */
                    if (mes.isRobot()) {
    ...
    private void handlePUTPIECE(StringConnection c, SOCPutPiece mes) {
        ...
        try {
            ...
            if (checkTurn(c, ga)) {
                 ...
                 * SOCGame.MAXPLAYERS; pn++) { SOCPlayer tmpPlayer = ga.getPlayer(pn);
                 * D.ebugPrintln("Player # "+pn); for (int i = 0x22; i < 0xCC; i++) { if
                ...
                switch (mes.getPieceType()) {
                    case SOCPlayingPiece.ROAD:
                        ...
                        if ((gameState == SOCGame.START1B) || (gameState == SOCGame.START2B)
                                || (gameState == SOCGame.PLACING_ROAD) || (gameState == SOCGame.PLACING_FREE_ROAD1)
                                || (gameState == SOCGame.PLACING_FREE_ROAD2)) {
                            if (player.isPotentialRoad(coord)) {
                                ...
                                /*
                                 * if (D.ebugOn) { D.ebugPrintln("AFTER"); for (int pn = 0; pn <
                                 * SOCGame.MAXPLAYERS; pn++) { SOCPlayer tmpPlayer = ga.getPlayer(pn);
                                 * D.ebugPrintln("Player # "+pn); for (int i = 0x22; i < 0xCC; i++) { if
                                 * (tmpPlayer.isPotentialRoad(i))
                                 * D.ebugPrintln("### POTENTIAL ROAD AT "+Integer.toHexString(i)); } } }
                                ...
                                messageToGameWithMon(gaName,
                                        new SOCGameTextMsg(gaName, SERVERNAME, plName + " built a road."));
                                messageToGameWithMon(gaName,
                                        new SOCPutPiece(gaName, player.getPlayerNumber(), SOCPlayingPiece.ROAD, coord));
                                gameList.releaseMonitorForGame(gaName);
                                boolean toldRoll = sendGameState(ga, false);
                                ...
                                if (COLLECT_VALUE_FUNCTION_APPROX)
                                    messageToGame(ga.getName(),
                                            new SOCCollectData(ga.getName(), player.getPlayerNumber()));
                                ...
                                D.ebugPrintlnINFO("ILLEGAL ROAD: 0x" + Integer.toHexString(coord) + ": player "
                                        + player.getPlayerNumber());
                    ...
                    case SOCPlayingPiece.SETTLEMENT:
                        ...
                        if ((gameState == SOCGame.START1A) || (gameState == SOCGame.START2A)
                                || (gameState == SOCGame.PLACING_SETTLEMENT)) {
                            if (player.isPotentialSettlement(coord)) {
                                ga.putPiece(se); // Changes game state and (if game start) player
                                gameList.takeMonitorForGame(gaName);
                                messageToGameWithMon(gaName,
                                        new SOCGameTextMsg(gaName, SERVERNAME, plName + " built a settlement."));
                                messageToGameWithMon(gaName, new SOCPutPiece(gaName, player.getPlayerNumber(),
                                        SOCPlayingPiece.SETTLEMENT, coord));
                                gameList.releaseMonitorForGame(gaName);
                                broadcastGameStats(ga);
                                sendGameState(ga);
                                ...
                                D.ebugPrintlnINFO("ILLEGAL SETTLEMENT: 0x" + Integer.toHexString(coord) + ": player "
                                        + player.getPlayerNumber());
                    ...
                    case SOCPlayingPiece.CITY:
                        ...
                        if (gameState == SOCGame.PLACING_CITY) {
                            if (player.isPotentialCity(coord)) {
                                ...
                                gameList.takeMonitorForGame(gaName);
                                messageToGameWithMon(gaName,
                                        new SOCGameTextMsg(gaName, SERVERNAME, plName + " built a city."));
                                messageToGameWithMon(gaName,
                                        new SOCPutPiece(gaName, player.getPlayerNumber(), SOCPlayingPiece.CITY, coord));
                                gameList.releaseMonitorForGame(gaName);
                                broadcastGameStats(ga);
                                sendGameState(ga);
                                ...
                                D.ebugPrintlnINFO("ILLEGAL CITY: 0x" + Integer.toHexString(coord) + ": player "
                                        + player.getPlayerNumber());
                ...
                try {
                    ...
                    if (ga.canMoveRobber(player.getPlayerNumber(), mes.getCoordinates())) {
                        ...
                        messageToGame(gaName,
                                new SOCMoveRobber(gaName, player.getPlayerNumber(), mes.getCoordinates()));
     ...
     * ignored. {@link #readyGameAskRobotsJoin(SOCGame, StringConnection[]) Ask some
     * robots} to fill empty seats, or {@link #startGame(SOCGame) begin the game} if
     * no robots needed.
     *
     * @param c   the connection that sent the message
     * @param mes the messsage
    ...
    private void handleSTARTGAME(StringConnection c, SOCStartGame mes) {
        if (c != null) {
            ...
            gamesParams.put(gn,
                    new StacGameParameters(mes.getLoadFlag(), mes.getFolder(), mes.getTurnNo(), mes.getStartingPlayer(),
                            mes.getLoadBoardFlag(), mes.getChatNegotiationsFlag(), mes.getFullyObservableFlag(),
                            mes.getObservableVPFlag()));
            ...
            D.ebugPrintlnINFO(
                    "--- Server: clearing trade response for all players (3)" + tradeResponsesString(ga.getName()));
            ...
            if (ga != null) {
                ...
                try {
                    if (ga.getGameState() == SOCGame.NEW) {
                        ...
                        if (seatsFull && (numPlayers < 2)) {
                            ...
                            if (robots.isEmpty()) {
                                if (numPlayers < SOCGame.MINPLAYERS) {
                                    messageToGame(gn, "No robots on this server, please fill at least "
                                ...
                                if (numEmpty > robots.size()) {
                                    String m;
                                    if (anyLocked)
                                        m = "Sorry, not enough robots to fill all the seats.  Only " + robots.size()
                                                + " robots are available.";
                                    else
                                        m = "Sorry, not enough robots to fill all the seats.  Lock some seats.  Only "
                                                + robots.size() + " robots are available.";
                                    messageToGame(gn, m);
                                } else {
                                    ga.setGameState(SOCGame.READY);

                                    /**
                                     * Fill all the unlocked empty seats with robots. Build a Vector of
                                     * StringConnections of robots asked to join, and add it to the
                                     * robotJoinRequests table.
                                     */
                                    try {
                                        ...
                                        messageToGame(gn, m);
                                        System.err.println(
                                                formattedDate() + ":" + "Robot-join problem in game " + gn + ": " + m);
                                    }
                                }
                            }
                        }
                        ...
                        /**
                         * If this doesn't need robots, then start the game. Otherwise wait for them to
                         * sit before starting the game.
                         */
                        if (seatsFull) {
                            startGame(ga);
                        }
    ...
    /**
     * Fill all the unlocked empty seats with robots, by asking them to join. Builds
     * a Vector of StringConnections of robots asked to join, and adds it to the
     * robotJoinRequests table. Game state should be READY. At most
     * {@link SOCGame#getAvailableSeatCount()} robots will be asked.
     * <P>
     * Called by {@link #handleSTARTGAME(StringConnection, SOCStartGame)
     ...
     * @param robotSeats If robotSeats is null, robots are randomly selected. If
     *                   non-null, a MAXPLAYERS-sized array of StringConnections.
     *                   Any vacant non-locked seat, with index i, is filled with
     *                   the robot whose connection is robotSeats[i]. Other indexes
     *                   should be null, and won't be used.
     *
     * @throws IllegalStateException    if {@link SOCGame#getGameState()
     *                                  ga.gamestate} is not READY, or if
     *                                  {@link SOCGame#getClientVersionMinRequired()
     *                                  ga.version} is somehow newer than server's
     *                                  version (which is assumed to be robots'
     *                                  version).
     * @throws IllegalArgumentException if robotSeats is not null but wrong length,
     *                                  or if a robotSeat element is null but that
     *                                  seat wants a robot (vacant non-locked).
     */
    private void readyGameAskRobotsJoin(SOCGame ga, StringConnection[] robotSeats, boolean noShuffle)
        ...
        if (ga.getClientVersionMinRequired() > Version.versionNumber())
            throw new IllegalStateException(
                    "SOCGame version somehow newer than server and robots, it's " + ga.getClientVersionMinRequired());
        ...
        int idx = 0;
        if (useRobotSelectionForExperiment) {
            // for training games, we're using the original bots, which are listed first in
            // the robots array
            ...
            if (ga != null) {
                ...
                try {
                    ...
                    if ((pl != null) && ga.canRollDice(pl.getPlayerNumber())) {
                        ...
                        /**
                         * Send roll results and then text to client. Client expects to see DiceResult
                         * first, then text message; to reduce visual clutter, SOCPlayerInterface.print
                         * expects text message to follow a certain format. If a 7 is rolled,
                         * sendGameState will also say who must discard (in a GAMETEXTMSG).
                         */
                        messageToGame(gn, new SOCDiceResult(gn, ga.getCurrentDice()));
                        messageToGame(gn, plName + " rolled a " + dice.getA() + " and a " + dice.getB() + ".");
                        // This is the wrong place to send this game state, as resources have not been
                        // allocated
                        ...
                        if (ga.getCurrentDice() != 7) {
                                ...
                                if (!ga.isSeatVacant(i)) {
                                    ...
                                    if (playerCon != null) {
                                        ...
                                        for (int res = SOCPlayerElement.CLAY; res <= SOCPlayerElement.WOOD; ++res)
                                            messageToPlayer(playerCon, new SOCPlayerElement(ga.getName(), i,
                                                    SOCPlayerElement.SET, res, resources.getAmount(res)));
                                        messageToGame(ga.getName(),
                                                new SOCResourceCount(ga.getName(), i, resources.getTotal()));
                                    }
                                } // if (! ga.isSeatVacant(i))
                            } // for (i)
                            ...
                            sendGameState(ga);
                            /*
                             * if (D.ebugOn) { for (int i=0; i < SOCGame.MAXPLAYERS; i++) { SOCResourceSet
                             * rsrcs = ga.getPlayer(i).getResources(); String resourceMessage =
                             * "PLAYER "+i+" RESOURCES: "; resourceMessage +=
                             * rsrcs.getAmount(SOCResourceConstants.CLAY)+" "; resourceMessage +=
                             * rsrcs.getAmount(SOCResourceConstants.ORE)+" "; resourceMessage +=
                             * rsrcs.getAmount(SOCResourceConstants.SHEEP)+" "; resourceMessage +=
                             * rsrcs.getAmount(SOCResourceConstants.WHEAT)+" "; resourceMessage +=
                             * rsrcs.getAmount(SOCResourceConstants.WOOD)+" "; resourceMessage +=
                             * rsrcs.getAmount(SOCResourceConstants.UNKNOWN)+" "; messageToGame(gn, new
                             * SOCGameTextMsg(gn, SERVERNAME, resourceMessage)); } }
                             */
                            if (COLLECT_VALUE_FUNCTION_APPROX)
                                messageToGame(ga.getName(),
                                        new SOCCollectData(ga.getName(), ga.getCurrentPlayerNumber()));
                             ...
                             */
                            for (int i = 0; i < ga.maxPlayers; i++) {
                                if ((!ga.isSeatVacant(i)) && (ga.getPlayer(i).getResources().getTotal() > 7)) {
                                    // Request to discard half (round down)
                                    StringConnection con = getConnection(ga.getPlayer(i).getName());
                                    if (con != null) {
                                        con.put(SOCDiscardRequest.toCmd(ga.getName(),
                                                ga.getPlayer(i).getResources().getTotal() / 2));
                                    }
                                }
                            }
                        }
            ...
            if (ga != null) {
                ...
                try {
                    ...
                    if (ga.canDiscard(pn, mes.getResources())) {
                        ...
                        if (gp.fullyObservable) {
                            reportRsrcGainLoss(gn, mes.getResources(), true, pn, -1, null, null); // report the exact
                                                                                                  // loss to everyone
                             ...
                             */
                            messageToGameExcept(gn, c, new SOCPlayerElement(gn, pn, SOCPlayerElement.LOSE,
                                    SOCPlayerElement.UNKNOWN, mes.getResources().getTotal()), true);
                        }
                        messageToGame(gn,
                                (String) c.getData() + " discarded " + mes.getResources().getTotal() + " resources.");
    ...
    /**
     * handle "end turn" message. This normally ends a player's normal turn (phase
     * {@link SOCGame#PLAY1}). On the 6-player board, it ends their placements
    ...
    /**
     * Pre-checking already done, end the current player's turn in this game. Alter
     * game state and send messages to players. Calls {@link SOCGame#endTurn()},
     * which may also end the game. On the 6-player board, this may begin the
     * {@link SOCGame#SPECIAL_BUILDING Special Building Phase}, or end a player's
     * placements during that phase. Otherwise, calls
     * {@link #sendTurn(SOCGame, boolean)} and begins the next player's turn.
     * <P>
     * Assumes:
     * <UL>
     ...
     * </UL>
     * <P>
     * As a special case, endTurn is used to begin the Special Building Phase during
     * the start of a player's own turn, if permitted. (Added in 1.1.09)
     *
     * @param ga Game to end turn
     * @param pl Current player in <tt>ga</tt>, or null. Not needed except in
     *           SPECIAL_BUILDING. If null, will be determined within this method.
     */
    private void endGameTurn(SOCGame ga, SOCPlayer pl) {
        ...
        if (ga.getGameState() == SOCGame.SPECIAL_BUILDING) {
            ...
            messageToGame(gname,
                    new SOCPlayerElement(gname, cpn, SOCPlayerElement.SET, SOCPlayerElement.ASK_SPECIAL_BUILD, 0));
        }
        ...
        /**
         * send new state number; if game is now OVER, also send end-of-game messages.
         */
        ...
        /**
         * Clear trade offers from our internal representation as well (necessary for
         * logging)
         */
        ...
        // before beginning a new turn clear responses (this covers the scenario when a
        // robot had its turn ended forcibly)
        ...
        if (ga.getGameState() == SOCGame.SPECIAL_BUILDING)
            messageToGame(gname, "Special building phase: " + ga.getPlayer(ga.getCurrentPlayerNumber()).getName()
                    + "'s turn to place.");
    }

    /**
     * Try to force-end the current player's turn in this game. Alter game state and
     * send messages to players. Will call {@link #endGameTurn(SOCGame, SOCPlayer)}
     * if appropriate. Will send gameState and current player (turn) to clients.
     * <P>
     * If the current player has lost connection, send the {@link SOCLeaveGame
     ...
     * @param plName Current player's name. Needed because if they have been
     *               disconnected by
    ...
    private boolean forceEndGameTurn(SOCGame ga, final String plName) {
        ...
        if (cp.hasAskedSpecialBuild()) {
            ...
            messageToGame(gaName,
                    new SOCPlayerElement(gaName, cpn, SOCPlayerElement.SET, SOCPlayerElement.ASK_SPECIAL_BUILD, 0));
        }
        ...
        if (resGainLoss != null) {
             ...
             * types/amounts. For discard, tell the discarding player's client that they
             * discarded the resources, tell everyone else that the player discarded unknown
             * resources.
             */
            if (!res.isLoss())
                ...
                if (!gp.fullyObservable) {
                    ...
                    messageToGameExcept(gaName, c, new SOCPlayerElement(gaName, cpn, SOCPlayerElement.LOSE,
                            SOCPlayerElement.UNKNOWN, totalRes), true);
                } else {// don't care if the player is connected as the below will send the message to
                        // all connected players
                    reportRsrcGainLoss(gaName, resGainLoss, true, cpn, -1, null, null); // if perfect information game
                                                                                        // report exact types/amounts to
                                                                                        // everyone
                }
                messageToGame(gaName, plName + " discarded " + totalRes + " resources.");
            }
        ...
        /**
         * For initial placements, we don't end turns as normal. (Player number may go
         * forward or backwards, new state isn't PLAY, etc.) Update clients' gamestate,
         * but don't call endGameTurn.
         */
        ...
        /**
         * If the turn can now end, proceed as if player requested it. Otherwise, send
         * current gamestate. We'll all wait for other players to send discard messages,
         * and afterwards this turn can end.
         */
        ...
        if (c != null) {
            ...
            if (ga != null) {
                ...
                try {
                    ...
                    /**
                     * remake the offer with data that we know is accurate, namely the 'from' datum
                     */
                    SOCPlayer player = ga.getPlayer((String) c.getData());
                    ...
                    if (player != null) {
                        ...
                        if (offer.getFrom() != current) {
                            ...
                            for (int i = 0; i < to.length; i++) {
                                if (to[i] && i != current) {
                                    ...
                                    // synchronization (P1 offers, P3 accepts, P2 counters after P1 has finished his
                                    // turn)
                                    legalOffer = false;
                                    break;
                                }
                        ...
                        if (legalOffer) {
                            // Determine whether this is an initial offer, necessary for logging - only
                            // necessary if the offer came from current player (otherwise we know it's
                            // counter)
                            boolean isInitialOffer = true;
                            if (player.getPlayerNumber() == ga.getCurrentPlayerNumber()) {
                                // check each player for an offer - if there is one on the table, this must be a
                                // counter
                                for (SOCPlayer p : ga.getPlayers()) {
                                    // NB: A player can't counter their own offer - that would be considered a new
                                    // initial offer
                                    if (p.getCurrentOffer() != null
                            ...
                            SOCTradeOffer remadeOffer;
                            {
                                ...
                                remadeOffer = new SOCTradeOffer(gaName, player.getPlayerNumber(), offer.getTo(),
                                        offGive, offGet);
                                // System.err.println("Make offer(" + ga.getPlayer(offer.getFrom()).getName() +
                                // "): " + offer);
                                ...
                                if (!StacRobotBrain.isChatNegotiation()) {
                                    if (!usingPersuasion) {
                                        db.logTradeEvent(player, mes, ga.getCurrentPlayerNumber(), isInitialOffer,
                                                false); // force-accept messages require trading via the dialogue
                                                        // manager; so it can't be one
                                    } else {
                                        db.logTradeEvent(player, mes, ga.getCurrentPlayerNumber(), isInitialOffer,
                                                new Persuasion(), player.getGame().getRoundCount()); // using
                                                                                                     // persuasions
                                                                                                     // require trading
                                                                                                     // via the dialogue
                                                                                                     // manager; so it
                                    ...
                                    // System.err.println(ga.getTurnCount() + " - observing trade event - MAKE
                                    // OFFER: " + mes.toString());
                                }
                            }
        ...
        if (c != null) {
            ...
            if (ga != null) {
                ...
                try {
                    ...
                    messageToGame(gaName,
                            new SOCClearOffer(gaName, ga.getPlayer((String) c.getData()).getPlayerNumber()));
        ...
        if (c != null) {
            ...
            if (ga != null) {
                ...
                if (player != null) {
                    ...
                    // System.err.println(ga.getTurnCount() + " - observing trade event - REJECT
                    // OFFER: " + rejectMessage.toString());
    ...
    private void handleACCEPTOFFER(StringConnection c, SOCAcceptOffer mes) {
        if (D.ebugOn) {
            ...
            D.ebugPrintlnINFO("ACCEPT OFFER -- " + offeringPlayer.getName() + " -- accept offer: "
                    + offeringPlayer.getCurrentOffer());
            // SOCPlayer acceptingPlayer = game.getPlayer(mes.getAcceptingNumber());
            // System.err.println("Accept offer (off: " + offeringPlayer.getName() + ", acc:
            // " + acceptingPlayer.getName() + "): " + offeringPlayer.getCurrentOffer());
        }
        if (c != null) {
            SOCGame ga = gameList.getGameData(mes.getGame());
    ...
    /**
     * NOTE: calling this method to execute a trade assumes the players' have either
     * accepted and/or made a new offer and that the player who initiated the offer
     * object has its currentOffer field set to the trade to be executed.
     *
            ...
            try {
                ...
                if (player != null) {
                    ...
                    if (canMakeTradeComment == null) {
                        ...
                        recordGameEvent(mes.getGame(), mes.toCmd());
                        if (!usingPersuasion) {
                            db.logTradeEvent(player, ga.getPlayer(offeringNumber), mes, ga.getCurrentPlayerNumber(),
                                    lastTradeOfferWasForceAccept);
                        } else {
                            db.logTradeEvent(player, ga.getPlayer(offeringNumber), mes, ga.getCurrentPlayerNumber(),
                                    lastTradeOfferPersuasion);
                        }
                        ...
                        // System.err.println(ga.getTurnCount() + " - Server sending message for
                        // accepted trade: " + mes.toString());
                        // newProt tradeResponses.put(ga.getName(),new
                        // StacTradeMessage[ga.maxPlayers]);//clear the trade responses after a trade
                        // was accepted
                        D.ebugPrintlnINFO("--- Server: clearing trade response after trade was executed for players "
                                + mes.getAcceptingNumber() + " and " + mes.getOfferingNumber()
                                + tradeResponsesString(gaName));
                        StacTradeMessage[] responses = tradeResponses.get(ga.getName());
                        responses[mes.getAcceptingNumber()] = null;
                        responses[mes.getOfferingNumber()] = null;
                    } else {
                        // send the message that the trade was not accepted - this will only matter to
                        // the initiator,
                        ...
                        if (D.ebugIsEnabled()) {
                            ...
                            D.ebugPrintlnINFO("INFO: Impossible trade between " + offeringNumber + " and "
                                    + acceptingNumber + ": " + offerString);
                        }
                        ...
                        if (StacRobotBrain.isChatNegotiation()) {
                            ...
                            while (connEnum.hasMoreElements()) {
                                ...
                                if (connName.equals(acceptingName)) {
                                    D.ebugPrintlnINFO("Sending extra warning about impossible trade to: "
                                            + (String) conn.getData() + " in addition to: " + (String) c.getData());
                                    messageToPlayer(conn, gaName, MSG_ILLEGAL_TRADE + " - " + canMakeTradeComment);
                                }
                            }
                        }

                        D.ebugPrintlnINFO("--- Server: clearing trade response for all players (6)"
                                + tradeResponsesString(gaName));
                        tradeResponses.put(ga.getName(), new StacTradeMessage[ga.maxPlayers]);// clear the trade
                                                                                              // responses after an
                                                                                              // illegal trade
                    }
                }
                    ...
                    if (checkTurn(c, ga)) {
                        if (ga.canMakeBankTrade(give, get)) {
                            ...
                            if ((scd != null) && scd.isRobot)
                                D.ebugPrintlnINFO(
                                        "ILLEGAL BANK TRADE: " + c.getData() + ": give " + give + ", get " + get);
                        }
                    } else {
                        messageToPlayer(c, ga.getName(), "It's not your turn.");
                    }
    ...
    /**
     * handle "build request" message. If client is current player, they want to buy
     * a {@link SOCPlayingPiece}. Otherwise, if 6-player board, they want to build
     * during the {@link SOCGame#SPECIAL_BUILDING Special Building Phase}.
     *
     * @param c   the connection that sent the message
     * @param mes the messsage
            ...
            if (isCurrent) {
                if ((ga.getGameState() == SOCGame.PLAY1) || (ga.getGameState() == SOCGame.SPECIAL_BUILDING)) {
                    switch (pieceType) {
                        case SOCPlayingPiece.ROAD:
                            ...
                            if (ga.couldBuildRoad(pn)) {
                                ...
                                messageToGame(gaName, new SOCPlayerElement(gaName, pn, SOCPlayerElement.LOSE,
                                        SOCPlayerElement.CLAY, 1));
                                messageToGame(gaName, new SOCPlayerElement(gaName, pn, SOCPlayerElement.LOSE,
                                        SOCPlayerElement.WOOD, 1));
                                sendGameState(ga);
                            } else {
                        ...
                        case SOCPlayingPiece.SETTLEMENT:
                            ...
                            if (ga.couldBuildSettlement(pn)) {
                                ...
                                gameList.takeMonitorForGame(gaName);
                                messageToGameWithMon(gaName, new SOCPlayerElement(gaName, pn, SOCPlayerElement.LOSE,
                                        SOCPlayerElement.CLAY, 1));
                                messageToGameWithMon(gaName, new SOCPlayerElement(gaName, pn, SOCPlayerElement.LOSE,
                                        SOCPlayerElement.SHEEP, 1));
                                messageToGameWithMon(gaName, new SOCPlayerElement(gaName, pn, SOCPlayerElement.LOSE,
                                        SOCPlayerElement.WHEAT, 1));
                                messageToGameWithMon(gaName, new SOCPlayerElement(gaName, pn, SOCPlayerElement.LOSE,
                                        SOCPlayerElement.WOOD, 1));
                                gameList.releaseMonitorForGame(gaName);
                                sendGameState(ga);
                        ...
                        case SOCPlayingPiece.CITY:
                            ...
                            if (ga.couldBuildCity(pn)) {
                                ...
                                messageToGame(ga.getName(), new SOCPlayerElement(ga.getName(), pn,
                                        SOCPlayerElement.LOSE, SOCPlayerElement.ORE, 3));
                                messageToGame(ga.getName(), new SOCPlayerElement(ga.getName(), pn,
                                        SOCPlayerElement.LOSE, SOCPlayerElement.WHEAT, 2));
                                sendGameState(ga);
                            } else {
                    ...
                    try {
                        ...
                        messageToGame(gaName, new SOCPlayerElement(gaName, pn, SOCPlayerElement.SET,
                                SOCPlayerElement.ASK_SPECIAL_BUILD, 1));
                        ...
                        switch (mes.getPieceType()) {
                            case SOCPlayingPiece.ROAD:
                                ...
                                if (ga.getGameState() == SOCGame.PLACING_ROAD) {
                                    ...
                                    messageToGame(gaName, new SOCPlayerElement(gaName, player.getPlayerNumber(),
                                            SOCPlayerElement.GAIN, SOCPlayerElement.CLAY, 1));
                                    messageToGame(gaName, new SOCPlayerElement(gaName, player.getPlayerNumber(),
                                            SOCPlayerElement.GAIN, SOCPlayerElement.WOOD, 1));
                                    sendGameState(ga);
                                } else {
                            ...
                            case SOCPlayingPiece.SETTLEMENT:
                                ...
                                if (ga.getGameState() == SOCGame.PLACING_SETTLEMENT) {
                                    ...
                                    gameList.takeMonitorForGame(gaName);
                                    messageToGameWithMon(gaName, new SOCPlayerElement(gaName, player.getPlayerNumber(),
                                            SOCPlayerElement.GAIN, SOCPlayerElement.CLAY, 1));
                                    messageToGameWithMon(gaName, new SOCPlayerElement(gaName, player.getPlayerNumber(),
                                            SOCPlayerElement.GAIN, SOCPlayerElement.SHEEP, 1));
                                    messageToGameWithMon(gaName, new SOCPlayerElement(gaName, player.getPlayerNumber(),
                                            SOCPlayerElement.GAIN, SOCPlayerElement.WHEAT, 1));
                                    messageToGameWithMon(gaName, new SOCPlayerElement(gaName, player.getPlayerNumber(),
                                            SOCPlayerElement.GAIN, SOCPlayerElement.WOOD, 1));
                                    gameList.releaseMonitorForGame(gaName);
                                    sendGameState(ga);
                                } else if ((ga.getGameState() == SOCGame.START1B)
                                        || (ga.getGameState() == SOCGame.START2B)) {
                            ...
                            case SOCPlayingPiece.CITY:
                                ...
                                if (ga.getGameState() == SOCGame.PLACING_CITY) {
                                    ...
                                    messageToGame(gaName, new SOCPlayerElement(gaName, player.getPlayerNumber(),
                                            SOCPlayerElement.GAIN, SOCPlayerElement.ORE, 3));
                                    messageToGame(gaName, new SOCPlayerElement(gaName, player.getPlayerNumber(),
                                            SOCPlayerElement.GAIN, SOCPlayerElement.WHEAT, 2));
                                    sendGameState(ga);
                                } else {
        ...
        try {
            ...
            if (checkTurn(c, ga)) {
                if (((ga.getGameState() == SOCGame.PLAY1) || (ga.getGameState() == SOCGame.SPECIAL_BUILDING))
                    ...
                    gameList.takeMonitorForGame(gaName);
                    messageToGameWithMon(gaName,
                            new SOCPlayerElement(gaName, pn, SOCPlayerElement.LOSE, SOCPlayerElement.ORE, 1));
                    messageToGameWithMon(gaName,
                            new SOCPlayerElement(gaName, pn, SOCPlayerElement.LOSE, SOCPlayerElement.SHEEP, 1));
                    messageToGameWithMon(gaName,
                            new SOCPlayerElement(gaName, pn, SOCPlayerElement.LOSE, SOCPlayerElement.WHEAT, 1));
                    ...
                    if (gp.fullyObservable)
                        ...
                        messageToGameExcept(gaName, c,
                                new SOCDevCard(gaName, pn, SOCDevCard.DRAW, SOCDevCardConstants.UNKNOWN), true);
                        ...
                        switch (mes.getDevCard()) {
                            case SOCDevCardConstants.KNIGHT:
                                ...
                                if (ga.canPlayKnight(player.getPlayerNumber())) {
                                    ...
                                    messageToGameWithMon(gaName, new SOCDevCard(gaName, player.getPlayerNumber(),
                                            SOCDevCard.PLAY, SOCDevCardConstants.KNIGHT));
                                    messageToGameWithMon(gaName,
                                            new SOCSetPlayedDevCard(gaName, player.getPlayerNumber(), true));
                                    messageToGameWithMon(gaName, new SOCPlayerElement(gaName, player.getPlayerNumber(),
                                            SOCPlayerElement.GAIN, SOCPlayerElement.NUMKNIGHTS, 1));
                                    gameList.releaseMonitorForGame(gaName);
                                    broadcastGameStats(ga);
                                    sendGameState(ga);
                            ...
                            case SOCDevCardConstants.ROADS:
                                ...
                                if (ga.canPlayRoadBuilding(player.getPlayerNumber())) {
                                    ...
                                    gameList.takeMonitorForGame(gaName);
                                    messageToGameWithMon(gaName, new SOCDevCard(gaName, player.getPlayerNumber(),
                                            SOCDevCard.PLAY, SOCDevCardConstants.ROADS));
                                    messageToGameWithMon(gaName,
                                            new SOCSetPlayedDevCard(gaName, player.getPlayerNumber(), true));
                                    messageToGameWithMon(gaName, new SOCGameTextMsg(gaName, SERVERNAME,
                                            player.getName() + " played a Road Building card."));
                                    gameList.releaseMonitorForGame(gaName);
                                    sendGameState(ga);
                            ...
                            case SOCDevCardConstants.DISC:
                                ...
                                if (ga.canPlayDiscovery(player.getPlayerNumber())) {
                                    ...
                                    gameList.takeMonitorForGame(gaName);
                                    messageToGameWithMon(gaName, new SOCDevCard(gaName, player.getPlayerNumber(),
                                            SOCDevCard.PLAY, SOCDevCardConstants.DISC));
                                    messageToGameWithMon(gaName,
                                            new SOCSetPlayedDevCard(gaName, player.getPlayerNumber(), true));
                                    messageToGameWithMon(gaName, new SOCGameTextMsg(gaName, SERVERNAME,
                                            player.getName() + " played a Year of Plenty card."));
                                    gameList.releaseMonitorForGame(gaName);
                                    sendGameState(ga);
                                } else {
                            ...
                            case SOCDevCardConstants.MONO:
                                ...
                                if (ga.canPlayMonopoly(player.getPlayerNumber())) {
                                    ...
                                    gameList.takeMonitorForGame(gaName);
                                    messageToGameWithMon(gaName, new SOCDevCard(gaName, player.getPlayerNumber(),
                                            SOCDevCard.PLAY, SOCDevCardConstants.MONO));
                                    messageToGameWithMon(gaName,
                                            new SOCSetPlayedDevCard(gaName, player.getPlayerNumber(), true));
                                    messageToGameWithMon(gaName, new SOCGameTextMsg(gaName, SERVERNAME,
                                            player.getName() + " played a Monopoly card."));
                                    gameList.releaseMonitorForGame(gaName);
                                    sendGameState(ga);
                                } else {
                            ...
                            default:
                                D.ebugPrintlnINFO("* SOCServer.handlePLAYDEVCARDREQUEST: asked to play unhandled type "
                                        + mes.getDevCard());
    ...
    /**
     * Deep copies the game information required for restarting from the current
     * game state and communicates the request to the participating players.
     *
     * @param c   the StringConnection for sending messages back to the client (not
     *            needed)
     * @param mes the message
     */
    private void handleGAMECOPY(StringConnection c, SOCGameCopy mes) {
        // System.out.println("Server: received copy request, sending copy request
        // back"); //-- for quick debugging
        String folderName = DeepCopy.SAVES_DIR + mes.getFolder();
        if (folderName.equals(DeepCopy.SAVES_DIR)) {
            // if its empty give it a name based on the game name and timestamp so it will
            // be unique
            Date date = new Date();
        ...
        messageToGame(mes.getGame(), new SOCGameCopy(mes.getGame(), folderName, mes.getPlayerNumber())); // we need a
                                                                                                         // new message
                                                                                                         // to keep the
                                                                                                         // new folder
                                                                                                         // name
    ...
    /**
     * Reads the game information from files, replaces the existing one and sends
     * the load request to the participating players;
     *
     * @param c   the StringConnection for sending messages back to the client (not
     *            needed here)
     * @param mes the message
    ...
    private void handleLOADGAME(StringConnection c, SOCLoadGame mes) {
        if (mes.getFolder().equals("")) {
            ...
            return; // if an empty folder name has been passed from the client don't initiate the
                    // load just inore message
        }
        loadGame(mes);
        messageToGame(mes.getGame(), mes);
        // System.out.println("Server: received load game message"); //--for quick
        // debugging
    }
    ...
    private void loadGame(SOCLoadGame mes) {
        ...
        // without the correct player names in the game, the server will "lose" the
        ...
        // hopefully these two arrays are in the same order based on their position on
        // the board
    ...
    /**
     * Change the robot flag of a player in order to (de)activate the end turn
     * thread. Also let everyone know if that player is a robot/human so the
     * negotiations will take place via the appropiate interface(i.e. chat for
     * robots and both chat and original trade interface for human players).
     *
     * @param c   the StringConnection for sending messages back to the client (not
     *            needed here)
     * @param mes the message
    ...
    /**
     * ---MG handle "player starts trading" message
     *
     * @param c   the connection
     * @param mes the message
     */
                ...
                try {
                    ...
                    if (checkTurn(c, ga)) {
                        ...
                        if (ga.canDoDiscoveryAction(mes.getResources())) {
                            ...
                            reportRsrcGainLoss(gaName, mes.getResources(), false, player.getPlayerNumber(), -1, message,
                                    null);
                ...
                try {
                    ...
                    if (checkTurn(c, ga)) {
                        if (ga.canDoMonopolyAction()) {
                            ...
                            final String resName = " " + SOCResourceConstants.resName(mes.getResource()) + ".";
                            ...
                            for (int i = 0; i < ga.maxPlayers; i++) {
                                 ...
                                 */
                                // Note that the previous logic was incorrect. Setting a resource to 0 could
                                // leave outstanding UNKNOWN resources which were the stolen type.
                                // Also, it inappropriately informed the rest of the game about how many of that
                                // resource the monopolizer had prior to the card.
                                // messageToGameWithMon(gaName, new SOCPlayerElement(gaName, i,
                                // SOCPlayerElement.SET, mes.getResource(),
                                // ga.getPlayer(i).getResources().getAmount(mes.getResource())));
                                // Instead, simply notify each player of what they lost, then notify everyone of
                                // what the monopolizer gained
                                messageToGameWithMon(gaName, new SOCPlayerElement(gaName, i, SOCPlayerElement.LOSE,
                                        mes.getResource(), monoPicks[i]));
                                total += monoPicks[i];
                            }
                            int playerNumber = ga.getPlayer(monoPlayerName).getPlayerNumber();
                            messageToGameWithMon(gaName, new SOCPlayerElement(gaName, playerNumber,
                                    SOCPlayerElement.GAIN, mes.getResource(), total));
                            gameList.releaseMonitorForGame(gaName);

                            /**
                             * now that monitor is released, notify the victim(s) of resource amounts taken,
     ...
     * vote. Otherwise, reset the game to a copy with same name and (copy of) same
     * players, new layout.
     * <P>
     ...
     * voting has begun.
     * <P>
     * If only one player remains (all other humans have left at end), ask them to
     * start a new game instead. This is a rare occurrence and we shouldn't bring in
     * new robots and all, since we already have an interface to set up a game.
     * <P>
     * If any human player's client is too old to vote for reset, assume they vote
     * yes.
     *
    ...
    private void handleRESETBOARDREQUEST(StringConnection c, SOCResetBoardRequest mes) {
        ...
        /**
         * Is voting already active from another player? Or, has this player already
         * asked for voting this turn?
         */
        ...
        /**
         * Is there more than one human player? Grab connection information for humans
         * and robots.
         */
        ...
        int numHuman = SOCGameBoardReset.sortPlayerConnections(ga, null, gameList.getMembers(gaName), humanConns,
                robotConns);
        ...
        if (numHuman < 2) {
            ...
            if (votingPlayers == 0) {
                ...
                messageToGameWithMon(gaName, new SOCGameTextMsg(gaName, SERVERNAME, ">>> " + (String) c.getData()
                ...
                // Put it to a vote
                messageToGameWithMon(gaName, new SOCGameTextMsg(gaName, SERVERNAME,
                        (String) c.getData() + " requests a board reset - other players please vote."));
                ...
                for (int i = 0; i < ga.maxPlayers; ++i)
                    if (humanConns[i] != null) {
                        if (humanConns[i].getVersion() >= 1100)
                            ...
                            ga.resetVoteRegister(ga.getPlayer((String) (humanConns[i].getData())).getPlayerNumber(),
                                    true);
                    }
            }
        }
     ...
     * If vote succeeded, go ahead and reset the game. If vote rejected, let
     * everyone know.
     *
    ...
    /**
     * process the "game option get defaults" message. Responds to client by sending
     * {@link SOCGameOptionGetDefaults GAMEOPTIONGETDEFAULTS}. All of server's known
     * options are sent, except empty string-valued options. Depending on client
     * version, server's response may include option names that the client is too
     * old to use; the client is able to ignore them.
     *
     * @param c   the connection
     * @param mes the message
     * @since 1.1.07
    ...
    private void handleGAMEOPTIONGETDEFAULTS(StringConnection c, SOCGameOptionGetDefaults mes) {
        if (c == null)
            return;
        c.put(SOCGameOptionGetDefaults.toCmd(SOCGameOption.packKnownOptionsToString(true)));
    }

    /**
     * process the "game option get infos" message; reply with the info, with one
     * {@link SOCGameOptionInfo GAMEOPTIONINFO} message per option keyname. Mark the
     * end of the option list with {@link SOCGameOptionInfo GAMEOPTIONINFO}("-"). If
     * this list is empty, "-" will be the only GAMEOPTIONGETINFO message sent.
     * <P>
     * We check the default values, not current values, so the list is unaffected by
     * cases where some option values are restricted to newer client versions. Any
     * option where opt.{@link SOCGameOption#minVersion minVersion} is too new for
    ...
    private void handleGAMEOPTIONGETINFOS(StringConnection c, SOCGameOptionGetInfos mes) {
        ...
        if (okeys != null) {
            for (int i = 0; i < okeys.size(); ++i) {
                ...
                if ((!alreadyTrimmedEnums) && (opt.enumVals != null) && (opt.optType != SOCGameOption.OTYPE_UNKNOWN)
    ...
    /**
     * process the "new game with options request" message. For messages sent, and
     * other details, see
     * {@link #createOrJoinGameIfUserOK(StringConnection, String, String, String, Hashtable)}.
     * <P>
     * Because this message is sent only by clients newer than 1.1.06, we definitely
     * know that the client has already sent its version information.
     *
     * @param c   the connection
     * @param mes the message
     * @since 1.1.07
    ...
    private void handleNEWGAMEWITHOPTIONSREQUEST(StringConnection c, SOCNewGameWithOptionsRequest mes) {
        ...
        createOrJoinGameIfUserOK(c, mes.getNickname(), mes.getPassword(), mes.getGame(), mes.getOptions());
    }
    ...
    private final void handleDEBUGFREEPLACE(StringConnection c, SOCDebugFreePlace mes) {
        ...
        if (didPut) {
            messageToGame(gaName, new SOCPutPiece(gaName, mes.getPlayerNumber(), pieceType, coord));
            if (ga.getGameState() >= SOCGame.OVER) {
    ...
    /**
     * ---MG handle "request to speak" message
     *
     * @param c   the connection
     * @param mes the message
     */
    ...
    /**
     * ---MG Send information to the clients that the state of the speaking queue
     * changed
     */
    private void speakingQueueChanged(SOCGame game) {
        // //send the old state of the speaking queue (because it still contains the new
        // current speaker as head of the list)
    ...
    private void handleCREATEACCOUNT(StringConnection c, SOCCreateAccount mes) {
        ...
        try {
            ...
            c.put(SOCStatusMessage.toCmd(SOCStatusMessage.SV_PROBLEM_WITH_DB, cliVers,
                    "Problem connecting to database, please try again later."));
            return;
        }
        ...
        try {
            success = db.createAccount(mes.getNickname(), c.host(), mes.getPassword(), mes.getEmail(),
                    currentTime.getTime());
        } catch (SQLException sqle) {
        ...
        if (success) {
            c.put(SOCStatusMessage.toCmd(SOCStatusMessage.SV_ACCT_CREATED_OK, cliVers,
                    "Account created for '" + mes.getNickname() + "'."));
        } else {
            c.put(SOCStatusMessage.toCmd(SOCStatusMessage.SV_ACCT_NOT_CREATED_ERR, cliVers,
                    "Account not created due to error."));
        }
    }

    /**
     * We received a message from a player client that confirms that a trade should
     * be executed.
     *
    ...
    private void handleCONFIRMTRADEANSWER(StringConnection c, StacConfirmTradeAnswer mes) {
        ...
        String playerName = (String) c.getData();
        SOCGame ga = gameList.getGameData(mes.getGame());
        int offeringPlayer = (Integer) map.get("offeringPlayer"); // ga.getPlayer((String)
                                                                  // map.get("offeringPlayer")).getPlayerNumber();
        int acceptingPlayer = (Integer) map.get("acceptingPlayer"); // ga.getPlayer((String)
                                                                    // map.get("acceptingPlayer")).getPlayerNumber();
        if (mes.getAnswer()) {
            ArrayList<String> traders = (ArrayList) map.get("players");
            if (!traders.contains(playerName)) {
                D.ebugERROR("*** Unexpected confirm trade answer message for game " + mes.getGame() + " from player "
                        + playerName);
                return;
            }
    ...
    /**
     * Client has been approved to join game; send the entire state of the game to
     * client. Unless <tt>isTakingOver</tt>, send client join event to other
     * players. Assumes NEWGAME (or NEWGAMEWITHOPTIONS) has already been sent out.
     ...
     * sent by PUTPIECE. See comments here for further details. The group of
     * messages sent here ends with GAMEMEMBERS, SETTURN and GAMESTATE.
     * <P>
     *
     ...
     * @param isTakingOver Client is re-joining; this connection replaces an earlier
     *                     one which is defunct because of a network problem. If
     *                     <tt>isTakingOver</tt>, don't send anything to other
     *                     players.
     *
    ...
    private void joinGame(SOCGame gameData, StringConnection c, boolean isReset, boolean isTakingOver) {
        ...
        if (!isReset) {
            ...
            // System.err.println("SERVER: joining client app data: " +
            // c.getAppData().toString());
        ...
        for (int i = 0; i < gameData.maxPlayers; i++) {
            /**
             * send the already-seated player information; if isReset, don't send, because
             * sitDown will be sent from resetBoardAndNotify.
             */
            if (!isReset) {
        ...
        for (int i = 0; i < gameData.maxPlayers; i++) {
            ...
            // This lets them see "their" pieces before sitDown(), if they rejoin at same
            // position.
             ...
             */
            c.put(SOCPlayerElement.toCmd(gameName, i, SOCPlayerElement.SET, SOCPlayerElement.ROADS,
                    pl.getNumPieces(SOCPlayingPiece.ROAD)));
            c.put(SOCPlayerElement.toCmd(gameName, i, SOCPlayerElement.SET, SOCPlayerElement.SETTLEMENTS,
                    pl.getNumPieces(SOCPlayingPiece.SETTLEMENT)));
            c.put(SOCPlayerElement.toCmd(gameName, i, SOCPlayerElement.SET, SOCPlayerElement.CITIES,
                    pl.getNumPieces(SOCPlayingPiece.CITY)));

            c.put(SOCPlayerElement.toCmd(gameName, i, SOCPlayerElement.SET, SOCPlayerElement.UNKNOWN,
                    pl.getResources().getTotal()));

            c.put(SOCPlayerElement.toCmd(gameName, i, SOCPlayerElement.SET, SOCPlayerElement.NUMKNIGHTS,
                    pl.getNumKnights()));
        ...
        /**
         * If we're rejoining and taking over a seat after a network problem, send our
         * resource and hand information.
         */
        if (isTakingOver) {
        ...
        /**
         * Almost done; send GAMEMEMBERS as a hint to client that we're almost ready for
         * its input. There's no new data in GAMEMEMBERS, because player names have
         * already been sent by the SITDOWN messages above.
         */
        try {
        ...
        c.put(SOCGameState.toCmd(gameName, gameData.getGameState()));
        D.ebugPrintlnINFO("*** " + c.getData() + " joined the game " + gameName + " from " + c.host());

        // messageToGame(gameName, new SOCGameTextMsg(gameName, SERVERNAME, n+" joined
        // the game"));
        /**
    ...
    private void sitDown(SOCGame ga, StringConnection c, int pn, boolean robot, boolean isReset) {
        if ((c != null) && (ga != null)) {
            ...
            try {
                ...
                if (requests != null) {
                    /**
                     * if the request list is empty and the game hasn't started yet, then start the
                     * game
                     */
                ...
                if (gamesParams.get(gaName) != null && !gamesParams.get(gaName).load) // if we are the first player than
                                                                                      // the params will be null, but we
                                                                                      // don't need to know any info
                                                                                      // about ourselves so ignore
                    /**
                     * send all the private information (only if we are starting a new game as these
                     * are already known if loading and will interfere with the normal progresion of
                     * the game)
                     */
     ...
     * Called from
     * {@link #sitDown(SOCGame, StringConnection, int, boolean, boolean)}.
     * <P>
    ...
    /**
     * The current player is stealing from another player. Send messages saying what
     * was stolen.
     *
     * @param ga   the game
    ...
    protected void reportRobbery(SOCGame ga, SOCPlayer pe, SOCPlayer vi, int rsrc) {
        if (ga != null) {
            ...
            /**
             * send the text messages: "You stole a sheep resource from viName." "peName
             * stole a sheep resource from you." "peName stole a resource from viName."
             */
            messageToPlayer(peCon, gaName, "You" + mes.toString() + viName + '.');
            messageToPlayer(viCon, gaName, peName + mes.toString() + "you.");
            messageToGameExcept(gaName, exceptions,
                    new SOCGameTextMsg(gaName, SERVERNAME, peName + " stole a resource from " + viName), true);
        }
    }

    /**
     * send the current state of the game with a message. Assumes current player
     * does not change during this state. If we send a text message to prompt the
     * new player to roll, also sends a RollDicePrompt data message. If the client
     * is too old (1.0.6), it will ignore the prompt.
     *
     * @param ga the game
     *
    ...
    /**
     * send the current state of the game with a message. Note that the current (or
     * new) player number is not sent here. If game is now OVER, send appropriate
     * messages.
     *
     ...
     *
     * @param ga             the game
     * @param sendRollPrompt If true, and if we send a text message to prompt the
     *                       player to roll, send a RollDicePrompt data message. If
     *                       the client is too old (1.0.6), it will ignore the
     *                       prompt.
     *
    ...
    protected boolean sendGameState(SOCGame ga, boolean sendRollPrompt) {
        ...
        if (ga.getGameState() == SOCGame.OVER) {
            /**
             * Before sending state "OVER", enforce current player number. This helps the
             * client's copy of game recognize winning condition.
             */
    ...
    protected void sendGameStateOVER(SOCGame ga) {
         ...
         * SOCGame.checkForWinner; that was already called before sendGameStateOVER.)
         */
        ...
        for (int i = 0; i < ga.maxPlayers; i++) {
            ...
            if (devCards.getNumVPCards() > 0) {
                ...
                for (int devCardType = SOCDevCardConstants.CAP; devCardType < SOCDevCardConstants.UNKNOWN; devCardType++) {
                    if ((devCards.getAmount(SOCDevCardSet.OLD, devCardType) > 0)
                            || (devCards.getAmount(SOCDevCardSet.NEW, devCardType) > 0)) {
                        if (vpCardCount > 0) {
         ...
         */
        {
            ...
            /**
             * Update each player's win-loss count for this session. Tell each player their
             * resource roll totals. Tell each player how long they've been connected.
     ...
     * {@link SOCPlayerElement} and {@link SOCGameTextMsg} messages. Trades are also
     * reported to robots by re-sending the accepting player's
     * {@link SOCAcceptOffer} message.
     *
     * @param ga        the game
     ...
     * {@link SOCPlayerElement} and {@link SOCGameTextMsg} messages.
     *
     * @param ga   the game
    ...
    protected void reportBankTrade(SOCGame ga, SOCResourceSet give, SOCResourceSet get) {
        if (ga != null) {
            ...
            final int giveTotal = give.getTotal(), getTotal = get.getTotal();
     ...
     * lost/gained by a second player. Sends PLAYERELEMENT messages, either to
     * entire game, or to player only. Builds the resource-amount string used to
     * report the trade as text. Takes and releases the gameList monitor for this
     * game.
     * <P>
     ...
     * @param rset          Resource set (from a roll, or the "give" or "get" side
     *                      of a trade). Resource type
     *                      {@link SOCResourceConstants#UNKNOWN} is ignored. Only
     *                      positive resource amounts are sent (negative is
     *                      ignored).
     * @param isLoss        If true, "give" ({@link SOCPlayerElement#LOSE}),
     *                      otherwise "get" ({@link SOCPlayerElement#GAIN})
     * @param mainPlayer    Player number "giving" if isLose==true, otherwise
     *                      "getting". For each nonzero resource involved,
     *                      PLAYERELEMENT messages will be sent about this player.
     * @param tradingPlayer Player number on other side of trade, or -1 if no second
     *                      player is involved. If not -1, PLAYERELEMENT messages
     *                      will also be sent about this player.
     * @param message       Append resource numbers/types to this stringbuffer,
     *                      format like "3 clay,3 wood"; can be null.
     * @param playerConn    Null or mainPlayer's connection; send messages here
     *                      instead of sending to all players in game. Because
     *                      trades are public, there is no such parameter for
     *                      tradingPlayer.
     *
     * @see #reportTrade(SOCGame, int, int)
     * @see #reportBankTrade(SOCGame, SOCResourceSet, SOCResourceSet)
     ...
     */
    private void reportRsrcGainLoss(final String gaName, final SOCResourceSet rset, final boolean isLoss,
     ...
     * @param ga the game
     *
     * @return true if it is the player's turn; false if another player's turn, or
     *         if this player isn't in the game
     */
    ...
    protected void startGame(SOCGame ga) {
        if (ga != null) {
            ...
            // See if the next game this use must play is the special case of a training
            // game.
            ...
            if (gp.load) {
                ...
                for (int i = 0; i < ga.maxPlayers; i++) {
                    if (!ga.isSeatVacant(i)) {
                        SOCPlayer pl = ga.getPlayer(i);
                        messageToGameWithMon(gaName, new SOCPlayerElement(gaName, i, SOCPlayerElement.SET,
                                SOCPlayerElement.ROADS, pl.getNumPieces(SOCPlayingPiece.ROAD)));
                        messageToGameWithMon(gaName, new SOCPlayerElement(gaName, i, SOCPlayerElement.SET,
                                SOCPlayerElement.SETTLEMENTS, pl.getNumPieces(SOCPlayingPiece.SETTLEMENT)));
                        messageToGameWithMon(gaName, new SOCPlayerElement(gaName, i, SOCPlayerElement.SET,
                                SOCPlayerElement.CITIES, pl.getNumPieces(SOCPlayingPiece.CITY)));
                 ...
                 */
                messageToGameWithMon(gaName,
                        new SOCGameTextMsg(gaName, SERVERNAME, "Randomly picking a starting player..."));
             ...
             */
            messageToGame(gaName, new SOCStartGame(gaName, false, gp.load, gp.folderName, gp.simulationDepth,
                    gp.playerToStart, gp.loadBoard, gp.chatNegotiations, gp.fullyObservable, gp.observableVP));

            /**
             * send whose turn it is (we don't want to remind a player that already knows
             * whose turn it is)
             */
            if (!gp.load)
                sendTurn(ga, false);
     ...
     * general outline; step 1 and 2 are done immediately here, steps 3 through n
     * are done (after robots are dismissed) within
     * {@link #resetBoardAndNotify_finish(SOCGameBoardReset, SOCGame)}.
     * <OL>
     * <LI value=1>Reset the board, remember player positions. If there are robots,
     * set game state to {@link SOCGame#READY_RESET_WAIT_ROBOT_DISMISS}.
     * <LI value=2a>Send ResetBoardAuth to each client (like sending JoinGameAuth at
     * new game) Humans will reset their copy of the game. Robots will leave the
     * game, and soon be requested to re-join. (This simplifies the robot client.)
     * If the game was already over at reset time, different robots will be randomly
     * chosen to join the reset game.
     * <LI value=2b>If there were robots, wait for them all to leave the old game.
     * Otherwise, (race condition) they may leave the new game as it is forming. Set
     * {@link SOCGame#boardResetOngoingInfo}. Wait for them to leave the old game
     * before continuing. The call will be made from
     ...
     * {@link #resetBoardAndNotify_finish(SOCGameBoardReset, SOCGame)}.
     * <LI value=3>Send messages as if each human player has clicked "join" (except
     * JoinGameAuth)
     * <LI value=4>Send as if each human player has clicked "sit here"
     * <LI value=5a>If no robots, send to game as if someone else has clicked "start
     * game", and set up state to begin game play.
     * <LI value=5b>If there are robots, set up wait-request queue
     * (robotJoinRequests). Game will wait for robots to send JOINGAME and SITDOWN,
     * as they do when joining a newly created game. Once all robots have re-joined,
     * the game will begin.
     * </OL>
     */
    private void resetBoardAndNotify(final String gaName, final int requestingPlayer) {
        /**
         * 1. Reset the board, remember player positions. Takes the monitorForGame and
         * (when reset is ready) releases it. If robots, resetBoard will also set
         * gamestate and boardResetOngoingInfo field.
         */
        ...
        // Announce who asked for this reset
        {
            ...
            messageToGameUrgent(gaName, ">>> Game " + gaName + " board reset by " + plName);
        }
        ...
        /**
         * Player connection data: - Humans are copied from old to new game - Robots
         * aren't copied to new game, must re-join
         */
         ...
         * Humans will reset their copy of the game. Robots will leave the game, and
         * soon will be requested to re-join.
         */
     ...
     * {@link #resetBoardAndNotify(String, int)}, after any robots have left the old
     * game.
     *
     * @param reBoard
     * @param reGame
    ...
    private void resetBoardAndNotify_finish(SOCGameBoardReset reBoard, SOCGame reGame) {
        ...
        /**
         * 5a. If no robots, send to game as if someone else has clicked "start game",
        ...
        if (!reBoard.hadRobots) {
            ...
            /**
             * 5b. If there are robots, set up wait-request queue (robotJoinRequests) and
             * ask robots to re-join. Game will wait for robots to send JOINGAME and
             * SITDOWN, as they do when joining a newly created game. Once all robots have
             * re-joined, the game will begin.
             */
            reGame.setGameState(SOCGame.READY);
            readyGameAskRobotsJoin(reGame, gameWasOverAtReset ? null : reBoard.robotConns, false);// reset is not
                                                                                                  // supported so don't
                                                                                                  // care about
                                                                                                  // noShuffle flag
        }
     ...
     * {@link SOCBoardLayout} or {@link SOCBoardLayout2}, depending on
    ...
    // {
    /*
     * SOCGameRecord gr = (SOCGameRecord)gameRecords.get(gn); SOCGameEventRecord ger
     * = currentGameEventRecord.myClone(); gr.addEvent(ger);
     */
    // }
    ...
    /*
     * private void writeGameRecord(String na, SOCGameRecord gr) { FileOutputStream
     * os = null; ObjectOutput output = null;
     *
     * try { Date theTime = new Date(); os = new
     * FileOutputStream("dataFiles/"+na+"."+theTime.getTime()); output = new
     * ObjectOutputStream(os); } catch (Exception e) { D.ebugPrintln(e.toString());
     * D.ebugPrintln("Unable to open output stream."); } try{
     * output.writeObject(gr); //
     * D.ebugPrintln("*** Wrote "+na+" out to disk. ***"); output.close(); } catch
     * (Exception e) { D.ebugPrintln(e.toString());
    ...
    protected void storeGameScores(SOCGame ga) {
        if (ga != null) {
            ...
            // D.ebugPrintln("allOriginalPlayers for "+ga.getName()+" :
            // "+ga.allOriginalPlayers());
            if ((ga.getGameState() == SOCGame.OVER) && (ga.allOriginalPlayers())) {
                ...
                try {
                    ...
                    // ---MG System.err.println("game scores, player 0: " +
                    // ga.getPlayer(0).getName());
                    // ---MG System.err.println("game scores, original players: " +
                    // ga.getPlayers());

                    // ---MG
                    // ---MG SOCDBHelper.saveGameScores(ga.getName(), ga.getPlayer(0).getName(),
                    // ga.getPlayer(1).getName(), ga.getPlayer(2).getName(),
                    // ga.getPlayer(3).getName(), (short) ga.getPlayer(0).getTotalVP(), (short)
                    // ga.getPlayer(1).getTotalVP(), (short) ga.getPlayer(2).getTotalVP(), (short)
                    // ga.getPlayer(3).getTotalVP(), ga.getStartTime());
                    ...
                    for (int i = 0; i < SOCGame.MAXPLAYERS_STANDARD; i++) {
                        ...
                        // if (player != null) {
                        // //System.err.println("SERVER: Storing trade reminder flag for player: " +
                        // player.getName());
    ...
    protected void recordGameEvent(String gameName, String event) {
        /*
         * FileWriter fw = (FileWriter)gameDataFiles.get(gameName); if (fw != null) {
         * try { fw.write(event+"\n"); //D.ebugPrintln("WROTE |"+event+"|"); } catch
         * (Exception e) { D.ebugPrintln(e.toString());
    ...
    /**
     * this is a debugging command that gives resources to a player. Format: rsrcs:
     * #cl #or #sh #wh #wo playername
     */
    protected void giveResources(StringConnection c, String mes, SOCGame game) {
        ...
        while (st.hasMoreTokens()) {
            if (resourceType <= SOCResourceConstants.WOOD) {
                ...
                // get all the of the line, in case there's a space in the player name ("robot
                // 7"),
        ...
        for (resourceType = SOCResourceConstants.CLAY; resourceType <= SOCResourceConstants.WOOD; resourceType++) {
            ...
            messageToGame(game.getName(), new SOCPlayerElement(game.getName(), pnum, SOCPlayerElement.GAIN,
    ...
    /**
     * this broadcasts game information to all people connected used to display the
     * scores on the player client
     */
    protected void broadcastGameStats(SOCGame ga) {
        /*
         * if (ga != null) { int scores[] = new int[SOCGame.MAXPLAYERS]; boolean
         * robots[] = new boolean[SOCGame.MAXPLAYERS]; for (int i = 0; i <
         * SOCGame.MAXPLAYERS; i++) { SOCPlayer player = ga.getPlayer(i); if (player !=
         * null) { if (ga.isSeatVacant(i)) { scores[i] = -1; robots[i] = false; } else {
         * scores[i] = player.getPublicVP(); robots[i] = player.isRobot(); } } else {
         * scores[i] = -1; } }
         *
     ...
     * ({@link SOCGame#isPractice} flag set) don't expire.
     *
     * @param currentTimeMillis The time when called, from
        ...
        try {
            for (Enumeration k = gameList.getGamesData(); k.hasMoreElements();) {
                ...
                if (gameExpir <= currentTimeMillis) {
                    ...
                    messageToGameUrgent(gameData.getName(), ">>> Less than " + minutes
    ...
    /**
     * Check for robot turns that have expired, and end them. They may end from
     * inactivity or from an illegal placement. Checks the
     * {@link SOCGame#lastActionTime} field.
     *
     * @param currentTimeMillis The time when called, from
     *                          {@link System#currentTimeMillis()}
        ...
        try {
            for (Enumeration k = gameList.getGamesData(); k.hasMoreElements();) {
                ...
                if (SOCGame.OVER <= ga.getGameState()) {
                    ...
                    ga.lastActionTime += (1000L * 60L * SOCGameListAtServer.GAME_EXPIRE_MINUTES);
                    continue;
                }
                final int cpn = ga.getCurrentPlayerNumber();
                if (cpn == -1)
                ...
                if (pl.getCurrentOffer() != null || ga.getGameState() == SOCGame.WAITING_FOR_DISCARDS) {
                    ...
                    final long tradeInactiveTime = currentTimeMillis - (1000L * ROBOT_FORCE_ENDTURN_TRADEOFFER_SECONDS);
    ...
    /**
     * Call the logger to initiate logging for the specified game and initialise the
     * parserIncrementString.
     *
     * @param gaName
     * @throws IOException
     */
    ...
    /**
     * Send a string to the logger and add it to the incrementally collected parser
     * string.
     *
    ...
    /**
     * Log the message with the logger and send it to the parser if it is a line
     * that would be written to a soclog file.
     *
     * @param recipients
     * @param message
     */
    ...
    /**
     * Strings for accumulating a string to be sent to the parser. There's one
     * string for each game running on the server. Game name is the key of the
     * HashMap.
     */
    ...
    /**
     * Add a new line to the "parser increment string", i.e. the string built up by
     * the server to be sent to the parser (at some point in the future).
     *
     * @param addStr
     */
    void addToParserString(String gaName, String addStr) {
        ...
        // get the parser increment string for the game in which this message is being
        // sent
    ...
    void addToParserXMLString(String gaName, SOCMessage message) {
        // get the parser increment XML string for the game in which this message is
        // being sent
    ...
    /**
     * Send a SOCMessage to the parser. Adds the message to the increment string and
     * then decides whether to trigger sending and receiving a string to/from the
     * parser.
     *
     * @param message
     */
    private void sendMessageToParser(SOCMessageForGame message) {
        ...
        if (message instanceof SOCGameTextMsg) {
            ...
            // Only call the parser if the text was sent by a human player; we have our own
            // internal handling of chat messages comig from robots
    ...
    /**
     * The actual interaction with the parser via ZMQ. Sends the incrementally built
     * up parser string, waits for the parser output and clears the parser string.
     *
    ...
    /**
     * Send the parse result to the clients playing the game. This information
     * should only be used by robot clients and ignored by clients for human
     * players.
     *
     * @param gaName the game's name
     ...
     * {@link SOCGameOption#setKnownOptionCurrentValue(SOCGameOption)} is called to
     * set them globally.
     *
     * @param args args as passed to main
     * @return Properties collection of args, or null for argument error. Will
     *         contain at least {@link #PROP_JSETTLERS_PORT},
    ...
    public static Properties parseCmdline_DashedArgs(String[] args) {
        ...
        ++aidx;
        argp.setProperty(SOCDBHelper.PROP_JSETTLERS_DB_USER, args[aidx]);
        ++aidx;
        argp.setProperty(SOCDBHelper.PROP_JSETTLERS_DB_PASS, args[aidx]);
        ++aidx;
     ...
     *
     * @param longFormat short or long? Long format gives details and also calls
    ...
    public static void printGameOptions() {
        ...
        if (optsVers > -1) {
            System.err.println("*** Note: Client version " + Version.version(optsVers)
    ...
    /**
     * Turns the Trade responses into a friendly String if these have been
     * initialised TODO: This method is duplicated on the client and server; better
     * make it static and move it to StacTradeMessage?
     *
     * @param gaName the name of the game
     * @return the trade responses as a String or a String specifying that these are
     *         not initialised
     */
     ...
     * SOCRobotClient in network games.
     * <P>
    ...
    private static class SOCPlayerLocalRobotRunner implements Runnable {
         ...
         * <P>
         * <b>Note:</b> If a bot is disconnected from the server, it's not removed from
         * this list, because the same bot will try to reconnect. To see if a bot is
         * connected, check {@link SOCServer#robots} instead.
         *
         * @since 1.1.13
         */
        ...
        /**
         * Create and start a robot client within a {@link SOCPlayerLocalRobotRunner}
         * thread. After creating it, {@link Thread#yield() yield} the current thread
         ...
         *                                can't be loaded. This can happen due to
         *                                packaging of the server-only JAR.
         * @throws LinkageError           for same reason as ClassNotFoundException
         */
        public static Thread createAndStartRobotClientThread(SOCRobotFactory factory, final String rname,
                final String strSocketName, final int port, SimpleAgent deeptrader)
    ...
    /**
     * Force-end this robot's turn. Done in a separate thread in case of deadlocks.
    ...
    private class ForceEndTurnThread extends Thread {
        ...
        public void run() {
            ...
            String msg = "For robot " + rname + ": force end turn in game " + ga.getName() + " cpn=" + plNum + " state "
                    + ga.getGameState();
    ...
    /**
     * Retrieve the SOCGame object representing the game of the requested name on
     * the active server. Used to get around the client-server architecture when
     * running local experiments (eg the omniscient robot brain) Method is strongly
     * discouraged, and is commented out. Was used for resource omniscience, now
     * simulated using inter-agent resource communication. Uncomment if this needs
     * to be reverified.
     *
     * @param gaName
     * @return
     *
     *         public static SOCGame getGame(String gaName) { return
     *         activeServer.gameList.getGameData(gaName); }
     */

    /**
     * Writes to the Postgresql database the game event (i.e. action) and the state
