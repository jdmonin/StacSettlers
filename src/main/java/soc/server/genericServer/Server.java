...
{
    // Store a collection of all threads started by the object.  Use a weak map so that threads are free to dispose themselves if they finish.
    //  NB: Value is irrelevant here, but I don't think there's a basic WeakSet object
    ...
    // Hate this being public, but the way threads are used is nasty all over the place.
    public synchronized static void trackThread(Thread t, Object o) {
        startedThreads.put(t, o);
    }
    
    /**
     * also displays which threads are interrupted, alive and the total number of each
     */
    public synchronized static void displayTrackedRunningThreads(){
        // Now interrupt all threads to force them to stop and clean themselves up.
        Set<Thread> threads = new HashSet<Thread>(startedThreads.keySet());
        int alive = 0;
        int interrupted = 0;
        int both = 0;
        int notCollected = 0;
        ...
        for (Thread t : threads) {    
        	if(t.isAlive() && t.isInterrupted()){//this should be impossible, but I doubt my understanding of this system
        		System.out.println("Thread: " + t.getName() + "   is alive and interrupted");
        		both++;
        	}else if(t.isAlive()){
        		System.out.println("Thread: " + t.getName() + "   is alive");
        		alive++;
        	}else if(t.isInterrupted()){
        		System.out.println("Thread: " + t.getName() + "   is interrupted");
        		interrupted++;
    		}else{
        		System.out.println("Thread: " + t.getName() + "   has not been collected, why?");
        		notCollected++;
        		if(t.isDaemon())
        			daemon++;
    		}
        ...
        System.out.println("Both Threads: " + both);
        System.out.println("Interrupted Threads: " + interrupted);
        System.out.println("NotCollected Threads: " + notCollected);
        System.out.println("Out of which are daemon threads: " + daemon);
        System.out.println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
    }
    ...
    public Server(String stringSocketName)
    {
        ...
        //initMisc(); //we don't need version checking
    ...
    public void run()
    {
    	Thread.currentThread().setName("Server");//for debugging to check whether this thread gets collected or not
...
//        treater.start(); 
        ...
        while (isUp())
        {
            try
            {
                while (isUp())
                {
                    ...
                    if (port != -1)
                    {
                        startThread((Connection) con, this);
                    }
                    else
                    {
                        ((LocalStringConnection) con).setServer(this);
                        Thread t = new Thread((LocalStringConnection) con);
                        startThread(t, this); 
                    }
                    //addConnection(new StringConnection());
                    if(Thread.currentThread().isInterrupted()){//in case of forcibly stopping threads
                    	break;// I am unsure how safe this is
                    }
                }
            }
            catch (IOException e)
        ...
        try {
			ss.close(); //we should close this after server was stopped
		} catch (IOException e) {}
    ...
    public synchronized void stopServer()
    {
        ...
        conns.clear();
        //and don't forget to stop the socket that keeps the server in the run loop resulting in the server not being killed and other threads as well.
        try {
			ss.close();
		} catch (IOException e) {}
        
        //a daemon timer thread that gets started and is unaccounted for afterwards;
        utilTimer.purge();
        utilTimer.cancel();
        utilTimer = null;
        killAllThreadsCreatedBy(this);
    }
    
    public synchronized static void killAllThreads() {
        // Now interrupt all threads to force them to stop and clean themselves up.
        Set<Thread> threads = new HashSet<Thread>(startedThreads.keySet());
    ...
    /**
	* Interrupts all tracked threads by setting a flag. If the flag is not checked inside the run loop of the specific thread nothing will happen. 
	*/
    public synchronized static void killAllThreadsCreatedBy(Object o){
    	//kill threads left running by the server
    	if(o.getClass().equals(Server.class) || o.getClass().equals(SOCServer.class)){
	    	for(Entry<Thread,Object> entry : startedThreads.entrySet()){
	    		if(entry.getValue()!=null && entry.getValue().equals(o)){
	    				entry.getKey().interrupt();
	    		}
	    	}
    	}
    	//also kill threads left running by the robot clients
    	if(o.getClass().equals(SOCRobotClient.class))
    	for(Entry<Thread,Object> entry : startedThreads.entrySet()){
    		if(entry.getValue()!=null && entry.getValue().equals(o)){
    			entry.getKey().interrupt();
    		}
    	...
    	System.gc(); //hint to run gc when possible
        ...
        if (D.ebugIsEnabled())
        {
            ...
            if ((cerr == null) || (! (c instanceof SocketTimeoutException)) || ! c.wantsHideTimeoutMessage())
            {
                if (cKey != null)
                {
                    ...
                    D.ebugPrintlnINFO(c.host() + " left (" + connectionCount() + ")  " + (new Date()).toString() + ((cerr != null) ? (": " + cerr.toString()) : ""));
                }
            }
        }
    }
    ...
    public void addConnection(StringConnection c)
    {
        ...
        if (connAccepted)
        {
            ...
            D.ebugPrintlnINFO(c.host() + " came but rejected (" + connectionCount() + ")  " + (new Date()).toString());
        }
    }
    	...
    	synchronized (unnamedConns) {
	        ...
	        {	
	        	...
	        	((StringConnection) e.nextElement()).put(m);
					...
					ex.printStackTrace();
					//What happens here is that the broadcast is done on unsynchronized vectors, so when it tries to send a message to an unnamed connection 
					//that was just removed, a NoSuchElementException is thrown. Trying to synchronise for now, but catch and print out this exception 
					//in case it recurs; catching and ignoring it does not affect the system, however not catching it stops the simulation
				}
	        }
    	}
    }
    ...
    class Treater extends Thread
    {
        ...
        public void run()
        {
            while (isUp())
            {
                ...
                {
                    System.err.println("Exception in treater (processCommand) - " + c.str);
                    e.printStackTrace();
                }
    ...
    protected class ConnExcepDelayedPrintTask extends TimerTask
    {
        ...
        public void run()
        {
            ...
            {
                D.ebugPrintlnINFO(connHost + " came (" + connectionCount() + ")  " + (new Date(thrownAt)).toString());
                cliConnDisconPrintsPending.remove(arrivingConn);
            } else {
                D.ebugPrintlnINFO(connHost + " left (" + connectionCount() + ")  " + (new Date(thrownAt)).toString() + ((excep != null) ? (": " + excep.toString()) : ""));
                cliConnDisconPrintsPending.remove(connData);
            }
            arrivingConn = null;//deallocate just in case this lingers around after running
        }
