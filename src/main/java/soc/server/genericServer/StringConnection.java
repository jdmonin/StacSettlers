    ...
    extends Connection implements Runnable
{
    ...
    public String readNext() throws EOFException, IllegalStateException
    {
        ...
        synchronized (in)
        {
            while (in.isEmpty())
            {
                ...
                try
                {
                    in.wait(100);//a timeout is required, otherwise this connection will run until notified otherwise and won't check for eof
                }
    ...
    public void run()
    {
        ...
        ourServer.removeConnection(this, false);//remove now if interrupted else this will do nothing
        ourServer = null; //also deallocate just to be sure
    }
