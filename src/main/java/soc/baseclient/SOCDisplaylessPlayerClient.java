...
public class SOCDisplaylessPlayerClient implements Runnable
{
     ...
     */
    public Hashtable<String, SOCGame> games = new Hashtable<String, SOCGame>();
    ...
    public void run()
    {
        ...
        catch (IOException e)
        {
            if (! connected)
            {
            	kill();// in case we are returning earlier due to the client being disconnected(this shouldn't happen as we are only running locally,but just in case)
                return;
            }
            ...
            // This is expected when the server shuts down.  The original implementation threw an exception and hence ended the thread anyway,
            //   this just makes it cleaner.
        }

        kill();// threads started by the child of this class are not actually cleaned up
    }
    ...
    */
    ...
    /** destroy the applet because the above is not called */
    public void kill(){
        SOCLeaveAll leaveAllMes = new SOCLeaveAll();
        put(leaveAllMes.toCmd());
        disconnect();
