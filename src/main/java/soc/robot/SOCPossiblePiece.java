...
public abstract class SOCPossiblePiece implements Serializable
{
     ...
     * @param s toString representation of a SOCPossiblePiece
     * @return SOCPossiblePiece represented by s, with player=null
     */
    public static SOCPossiblePiece parse(String s) {
        // Wrap in a try-catch - if anything goes wrong, we have a malformed string (or none at all) and should return null
        try {
            // split by |
            String[] pieces = s.split("\\|");
            int pieceType = Integer.parseInt(pieces[0].split("=")[1]);
            String coordStr = pieces[2].split("=")[1];
            int coord = Integer.parseInt(coordStr, 16);
     ...
     * @return
     */
    public abstract SOCResourceSet getResourceCost();
   
    /* NB: This caused problems with a place in the code that seems to insist on 
        a stricter (implied) definition of equality.  Be careful!
    public boolean equals(Object o) {
        if (! (o instanceof SOCPossiblePiece)) {
            return false;
        }
        SOCPossiblePiece p = (SOCPossiblePiece) o;
        return (pieceType == p.pieceType && coord == p.coord);
    }
    */
}
