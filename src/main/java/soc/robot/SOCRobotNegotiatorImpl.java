...
public class SOCRobotNegotiatorImpl extends SOCRobotNegotiator<SOCBuildPlanStack> {
   ...
   /**
     ...
     */
    public int considerOffer(SOCTradeOffer offer, final int receiverNum)
    {
        ///
     ...
     */
    private int getETAToTargetResources
        ...
        if (!match)
        {
            ...
            if (game.getCurrentPlayerNumber() == ourPlayerNumber)
            {
                ...
                for (int i = 0; i < game.maxPlayers; i++)
                {
                    ...
                    if ((i != ourPlayerNumber) && isSellingResource[i][neededResource] &&
                            (! game.isSeatVacant(i)) &&
