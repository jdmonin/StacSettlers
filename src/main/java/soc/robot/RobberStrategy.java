...
public class RobberStrategy {
   ...
   public int getBestRobberHex(SOCGame game, SOCPlayer ourPlayerData, HashMap playerTrackers)
   {
       ...
       log.ebugPrintlnINFO("%%% MOVEROBBER");
       ...
       while (trackersIter.hasNext())
       {
           ...
           log.ebugPrintlnINFO("%%%%%%%%% TRACKER FOR PLAYER " + tracker.getPlayer().getPlayerNumber());

           try
           {
               tracker.recalcWinGameETA();
               winGameETAs[tracker.getPlayer().getPlayerNumber()] = tracker.getWinGameETA();
               log.ebugPrintlnINFO("winGameETA = " + tracker.getWinGameETA());
           }
           catch (NullPointerException e)
           {
               log.ebugPrintlnINFO("Null Pointer Exception calculating winGameETA");
       ...
       for (int pnum = 0; pnum < game.maxPlayers; pnum++)
       {
           if (! game.isSeatVacant(pnum))
           {
               ...
               {
                   // The first pick
                   log.ebugPrintlnINFO("Picking a robber victim: pnum=" + pnum);
                   victimNum = pnum;
               }
       ...
       for (int i = 0; i < 19; i++)
       {
           ...
           if ((hexes[i] != robberHex) && (ourPlayerData.getNumbers().getNumberResourcePairsForHex(hexes[i]).isEmpty()))
           {
               ...
               log.ebugPrintlnINFO("total Speed = " + totalSpeed);

               if (totalSpeed > worstSpeed)
               {
                   bestHex = hexes[i];
                   worstSpeed = totalSpeed;
                   log.ebugPrintlnINFO("bestHex = " + Integer.toHexString(bestHex));
                   log.ebugPrintlnINFO("worstSpeed = " + worstSpeed);
               }
           }
       }
       ...
       while ((bestHex == robberHex) && (ourPlayerData.getNumbers().getNumberResourcePairsForHex(hexes[bestHex]).isEmpty()))
       {
           ...
           log.ebugPrintlnINFO("%%% random pick = " + Integer.toHexString(bestHex));
       }
