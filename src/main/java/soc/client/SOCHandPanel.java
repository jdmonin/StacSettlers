...
/*package*/ class SOCHandPanel extends JPanel
    implements ActionListener, MouseListener, ColorSquareListener
{
    /**
    ...
    protected static final String SEND = "Register Trade"; //strings.get("hpan.trade.offer");
    ...
    public SOCHandPanel(final SOCPlayerInterface pi, final SOCPlayer pl, final boolean isInteractive)
    {
        ...
        {
            vpSq.setToolTipHighWarningLevel(vp_close_to_win, game.getVpWinner() - 2);  // (win checked in SOCGame.checkForWinner)
        } else {
            vpSq.setToolTipHighWarningLevel(vp_close_to_win, game.getVpWinner() - 3);
        }
        add(vpSq);
...
//            speakingQueueSq.setVisible(seatTaken); //---MG
    ...
    public void rejectOfferShowNonClient()
    {
        ...
        offerPanel.setVisible(false);
        counterOfferPanel.setVisible(false);
        messagePanel.setText("Trade rejected!");//---MG  strings.get("base.no.thanks.sentenc"));  // "No thanks."
...
//            speakingQueueSq.setVisible(! counterIsShowing); //---MG
    ...
    private void updateRollDoneBankButtons()
    {
        ...
        save.setEnabled((gs == SOCGame.ROLL_OR_CARD) || (gs == SOCGame.PLAY1));
