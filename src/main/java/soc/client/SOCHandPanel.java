 ...
 */
public class SOCHandPanel extends Panel implements ActionListener, ColorSquareListener
{
    ...
    protected static final String SEND = "Register Trade"; //"Offer";
    ...
    protected static final String SIMULATE = "Sim"; //for running MCTS
    
    //---MG
    protected static final String REGISTER_TRADE_BUT = "Register a Trade";
    protected static final String BANK_TRADE_BUT = "Trade with Bank/Port";
    protected static final String CANCEL_TRADE_BUT = "Cancel";
    ...
    protected Label speakingQueueLab; //---MG
    protected ColorSquare speakingQueueSq; //---MG
    ...
    /**
     * ---MG
     * Determine which Trade Button should be visible
     */
    protected static int REGISTER_TRADE = 1;    
    protected static int BANK_TRADE = 2;
    ...
    /**
     * Checkbox for muting sound during development.
     */
    protected ColorSquare muteSound;
    protected Label muteLab;
    ...
    /**
     * ---MG
     * variable indicating whether the player has agreed on at least one trade prior to registering it with the system
     * set by a confirmation dialogue which pops up when pressing the "Register a Trade" button until until it is answered with "Yes"
     */
    protected boolean playerHasAgreedOnAtLeastOneTrade;
    ...
    protected void creation(SOCPlayerInterface pi, SOCPlayer pl, boolean in)
    {
        ...
        if (game.getVpWinner() <= game.getVpWinner() + 2)
        {
            vpSq.setTooltipHighWarningLevel("Close to winning", game.getVpWinner() - 2);  // (win checked in SOCGame.checkForWinner)
        } else {
            vpSq.setTooltipHighWarningLevel("Close to winning", game.getVpWinner() - 3);
        }
        add(vpSq);
...
//        }
        ...
        add(claySq);
        //---MG -- we don't allow offering trades with a right-click on a player's resource squares
        ...
        if (playerTradingDisabled)
        {
            ...
            for (int pn = 0; pn < game.maxPlayers; pn++)
            {
                if (pn != player.getPlayerNumber())
                {
                    ...
                    playerSend[cnt].setBoolValue(false); //---MG true);
                    add(playerSend[cnt]);
                    playerSend[cnt].setSquareListener(this); //---MG
                    cnt++;
                }
            }
        }  // if(playerTradingDisabled)
        ...
        muteSound = new ColorSquare(ColorSquare.CHECKBOX, true, ColorSquare.GREY);
        muteLab = new Label(" Mute: ");
        muteSound.setSquareListener(this);
        add(muteLab);
        add(muteSound);
        ...
        //---MG
        //update the hand panel interface to initial settings
        showRegisterTradeButs();
    }
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        else if (target == DONE)
        {
        	...
        	save.setEnabled(false); //---MD
            client.endTurn(game);
            //---MG
//            showRegisterTradeBut = true;
//            updateRegisterTradeBut(SHOW_REGISTER_TRADE_BUTS);
            showRegisterTradeButs();
        }
        ...
        else if (target == CLEAR)
        {
            ...
            //---MG
//        	showRegisterTradeBut = true;
//        	updateRegisterTradeBut(CLEAR_TRADE_AMOUNTS);
            clearTradeAmounts();
        }
        else if (target == BANK)
        {
        ...
        else if (target == SEND)
        {
            ...
            if (playerIsCurrent)
            	save.setEnabled(false);//do not allow saving during trades
            ...
            if (game.getGameState() == SOCGame.PLAY1)
            {
                ...
                //---MG
                //we want to allow partial offers
...
//                }
        ...
        //---MG
        else if (target == REGISTER_TRADE_BUT)
        {
            if (playerIsCurrent)
            	save.setEnabled(false);//do not allow saving during trades
            //---MG
        	//look at http://docs.oracle.com/javase/6/docs/api/javax/swing/JOptionPane.html and http://docs.oracle.com/javase/tutorial/uiswing/components/dialog.html for details on how to configure the dialogue
//        	if (playerHasAgreedOnAtLeastOneTrade == false) {
//        		Object[] options = { "No", "Yes" };
//	        	int result = JOptionPane.showOptionDialog(null, 
//	        			"Remember:\n" +
//	        			"You have to agree on a trade with another player\n" +
//	        			"via the online chat before registering it with the system.\n\n" +
//	        			"Have you agreed on a trade?", "Reminder",
//	        			JOptionPane.DEFAULT_OPTION, JOptionPane.WARNING_MESSAGE,
//	        			null, options, options[0]);
//	        	if (result == 0) {
//	        		playerHasAgreedOnAtLeastOneTrade = false;
//	            	//System.err.println(result + "no");
//	        	}
//	        	else if (result == 1) {
//	        		playerHasAgreedOnAtLeastOneTrade = true;
//	            	//System.err.println(result + "yes");
//	        	}
//        	}
//
...
//        	}
        ...
        //---MG
        else if (target == BANK_TRADE_BUT)
        {
        	hideRegisterTradeButs(BANK_TRADE);
        }
        //---MG
        else if (target == CANCEL_TRADE_BUT)
        {
            if (playerIsCurrent)
            	save.setEnabled(true);//if trade was canceled allow player to save game again
        	showRegisterTradeButs();
        }
        else if (target == SAVE){
        	if(!save.isEnabled())
        	...
        	//TODO: add mcts code for simulation testing
        }
        } catch (Throwable th) {
    ...
    //---MG
    /**
     * Called when a checkbox for the addressee of a trade changes its value. (Implementing the ColorSquareListener "protocol".)
     * @param sq
     * @param oldValue
     * @param newValue
     */
    public void squareChanged(ColorSquare sq, int oldValue, int newValue) {
//    	//Make sure that only one of the checkboxes is selected 
//    	if (newValue == 1) {
//    		for (int i = 0; i < (game.maxPlayers - 1); i++)
//    		{
//    			if (playerSend[i].getBoolValue() == true && playerSend[i] != sq)
//    				playerSend[i].setBoolValue(false);
//    		}
//        }
    }
    ...
    {
        D.ebugPrintlnINFO("*** addSeatLockBut() ***");
        D.ebugPrintlnINFO("seatLockBut = " + sittingRobotLockBut);
    ...
    public void removePlayer()
    {
        ...
        speakingQueueLab.setVisible(false); //---MG
        speakingQueueSq.setVisible(false); //---MG
    ...
    public void addPlayer(String name)
    {
        ...
        {
            D.ebugPrintlnINFO("SOCHandPanel.addPlayer: This is our hand");
...
//                speakingQueueLab.setVisible(true); //---MG
//                speakingQueueSq.setVisible(true); //---MG
            }
            ...
            if (resourceTradeMenu != null)
            {
                ...
                for (int i = 0; i < resourceTradeMenu.length; ++i)
                {
                    if (resourceTradeMenu[i] != null)
                    {
                        //resourceTradeMenu[i].destroy(); //this line causes issues with the replay client we already replace it with null so no need to destroy
                        resourceTradeMenu[i] = null;
                    }
                }
            }
...
//            //---MG
//            showTradeButton();
//            registerTradeBut.setVisible(true);
            save.setVisible(true); //---MD show buttons on our hand panel
            load.setVisible(true); //---MD
            load.setEnabled(true); //can load at any time
            sim.setVisible(true);
            sim.setEnabled(true);
            
            muteLab.setVisible(true);
            muteSound.setEnabled(true);
            muteSound.setVisible(true);
            muteSound.setBoolValue(false);
        ...
        else
        {
        	...
        	//if it is a perfect information game, display the player's hand
        	if(client.gamesParams.get(game.getName()) != null && client.gamesParams.get(game.getName()).fullyObservable){
                claySq.setVisible(true);
                clayLab.setVisible(true);
                oreSq.setVisible(true);
                oreLab.setVisible(true);
            ...
            D.ebugPrintlnINFO("**** SOCHandPanel.addPlayer(name) ****");
            D.ebugPrintlnINFO("player.getPlayerNumber() = " + player.getPlayerNumber());
            D.ebugPrintlnINFO("player.isRobot() = " + player.isRobot());
            D.ebugPrintlnINFO("game.isSeatLocked(" + player.getPlayerNumber() + ") = " + game.isSeatLocked(player.getPlayerNumber()));
            D.ebugPrintlnINFO("game.getPlayer(client.getNickname()) = " + game.getPlayer(client.getNickname()));
            ...
            {
                //this is disabled for the STAC evaluation; so, games can still be observed but not seats cannot be taken over
//                addTakeOverBut();
            }
...
//            speakingQueueSq.setVisible(true); //---MG
    ...
    public void updateAtTurn()
    {
        ...
        //---MG
//        updateRegisterTradeBut();
        showRegisterTradeButs(); // this updates the buttons to be enabled
    ...
    public void updateAtPlay1()
    {
       ...
       //---MG
       showRegisterTradeButs(); // updates the trade buttons so that they are enabled
    }

    /**
     * Enable, disable the correct buttons after loading a saved game
     */
    ...
    /**
     * ---MG
     * Shows the "Register Trade" button in the Player's area and hides the other trade buttons.
     */
    public void showRegisterTradeButs()
    {
//    	registerTradeBut.setVisible(true);
//    	bankTradeBut.setVisible(true);

//    	if (playerIsCurrent && (game.getGameState() == SOCGame.PLAY1)) {
//        	registerTradeBut.setEnabled(true);
//        	bankTradeBut.setEnabled(true);
//        } else {
//        	registerTradeBut.setEnabled(false);
//        	bankTradeBut.setEnabled(false);
//        }
...
//    	clearOfferBut.setVisible(false);
//    	bankBut.setVisible(false);
//    	cancelTradeBut.setVisible(false);
//        for (int pl = 0; pl < game.maxPlayers-1; pl++) {
//        	playerSend[pl].setVisible(false);
//        }
    }
    
    /**
     * ---MG
     * Hides the "Register Trade" button in the Player's area and shows the other trade buttons.
     * 
     * @param mode  show the trade panel for registering a trade with another player or for trading with the bank/port 
     */
    public void hideRegisterTradeButs(int mode)
    {
    	registerTradeBut.setVisible(false);
    	bankTradeBut.setVisible(false);
    	...
    	sqPanel.setVisible(true);
    	clearOfferBut.setVisible(true);
    	cancelTradeBut.setVisible(true);
//    	if (mode == REGISTER_TRADE) {
//    		offerBut.setVisible(true);
//    		bankBut.setVisible(false);
//            for (int pl = 0; pl < game.maxPlayers-1; pl++)
//            	playerSend[pl].setVisible(true);
//    	} else {
//    		offerBut.setVisible(false);
//    		bankBut.setVisible(true);
//            for (int pl = 0; pl < game.maxPlayers-1; pl++)
...
//    	}
    ...
    /**
     * ---MG
     * Reset the amounts shown by the square buttons in the trade area.
     */
    public void clearTradeAmounts() {
    	sqPanel.setValues(zero, zero);    	
    }
    
    /**
     * ---MG
     * Update the trade buttons depending on their current state; this is called from doLayout().
     */
    public void updateRegisterTradeBut() {
    	boolean flag = registerTradeBut.isVisible();
    	
    	registerTradeBut.setVisible(flag);
    	bankTradeBut.setVisible(flag);
    	giveLab.setVisible(!flag);
    	getLab.setVisible(!flag);
    	sqPanel.setVisible(!flag);
    	clearOfferBut.setVisible(!flag);
    	cancelTradeBut.setVisible(!flag);
...
//            speakingQueueLab.setVisible(seatTaken); //---MG
//            speakingQueueSq.setVisible(seatTaken); //---MG
        }
        ...
        if (inPlay)
        {
            ...
            if (currentOffer != null)
            {
                if (! (offerIsResetMessage || offerIsDiscardMessage))
                {
                    if (! playerIsClient)
                    {
                        //don't do this in a game that uses chat trading 
                        boolean chatTrading = game.isGameOptionSet("CN");
                        if (!chatTrading) {
    ...
    public void rejectOfferShowNonClient()
    {
        ...
        offer.setMessage("Trade rejected!");//---MG "No thanks.");
        if (offerHidesControls)
            hideTradeMsgShowOthers(false);
        offer.setVisible(true);
        //validate();
...
//            speakingQueueLab.setVisible(! counterIsShowing); //---MG
//            speakingQueueSq.setVisible(! counterIsShowing); //---MG
...
//        //---MG
//        // offerBut.bounds()
//        registerTradeBut.repaint(); //setBounds(inset - 2, tradeY - 2, 150, 80);
//        //System.err.println("aipdhgipagdh" + inset);
//        //System.err.println("repainting trade button");
    }

    /**
     * updates all values displayed on this handPanel
     */
    ...
    /**
     * method for forcing a specific number into the vp square. (only to be used by StacDBReplayClient)
     * @param newVP
     */
    public void updateJustVP(int newVP){
    	vpSq.setIntValue(newVP);
    }
    ...
    public void updateResourcesVP()
    {
        ...
        else
        {
            ...
            if(client.gamesParams.get(game.getName()).fullyObservable){
                updateValue(CLAY);
                updateValue(ORE);
                updateValue(SHEEP);
                updateValue(WHEAT);
...
//        //---MG
//        registerTradeBut.repaint();
//        System.err.println("repaint after update resource");
    }
    ...
    public void doLayout()
    {
        ...
        else
        {
            ...
            //NOTE: layout is done once before start of game, so first run must check that gp is not null;
            StacGameParameters gp = client.gamesParams.get(game.getName());
            ...
            if (playerIsClient)
            {
                ...
                //---MG
//                int sqW = fm.stringWidth(speakingQueueLab.getText().replace(' ','_'));  //Bug in stringWidth
//                speakingQueueLab.setBounds(inset + faceW + inset + 60, (inset + faceW) - lineH, vpW, lineH);
//                speakingQueueSq.setBounds(inset + faceW + inset + sqW + space + 56, (inset + faceW) - lineH, ColorSquare.WIDTH, ColorSquare.WIDTH);
////                System.err.println("position of speaker " + player.getName() + " queue: " + game.speakingQueue + " index: " + game.speakingQueue.indexOf(player.getName()));
                ...
                if(gp!=null && !gp.chatNegotiations){
                	...
                	sqPanel.setLocation(-100, -100);//somewhere away because it will be activated so many times during the game
...
//                bankBut.setBounds(tbX + tbW + space, tbY + lineH + space, tbW, lineH);
                ...
                if(gp!=null && !gp.chatNegotiations){
	                if (cancelTradeBut != null)
	                {
	                    if (game.maxPlayers == 4)
	                        cancelTradeBut.setBounds(tbX, tbY, tbW - 15, lineH);
	                    else  // 6-player: leave room for 5 checkboxes
	                        cancelTradeBut.setBounds(tbX, tbY, (2 * tbW) + space - (5 * (1 + ColorSquare.WIDTH)) - 15, lineH);
	                }
	                clearOfferBut.setBounds(tbX, tbY + lineH + space, tbW - 15, lineH);
	                //offer and bank button are drawn in the same location 
	                offerBut.setBounds(tbX + tbW + space - 15, tbY + lineH + space, tbW + 15, lineH);
	                //---MG changed for agents-only bankBut.setBounds(tbX + tbW + space - 15, tbY + lineH + space, tbW + 15, lineH);
	                bankBut.setBounds(tbX + tbW + space + 30, tbY + lineH + space, tbW + 15, lineH);
                }
                
                if (! playerTradingDisabled && gp!=null && !gp.chatNegotiations)
                {
                    if (game.maxPlayers == 4)
                    {
                ...
                //Draw the information about the hand depending on whether we're used in SOCPlayerClient or SOCReplayClient
                //and whether it's the chat trading or the old trade interface.
                if (gp!=null && gp.chatNegotiations) {
                    int dcW = fm.stringWidth("Dev._Cards:_") + 2;  // +2 because Label text does not start at pixel column 0
                    int roadsW = fm.stringWidth("Roads:_") + 2;  // +2 because Label text does not start at pixel column 0
                    int knW = fm.stringWidth("Soldiers:") + 2;
                            
                    if (client instanceof SOCReplayClient) {
                        knightsLab.setBounds(inset, tradeY + lineH + space, knightsW, lineH);
                        knightsSq.setBounds(inset + dcW, tradeY + lineH + space, ColorSquare.WIDTH, ColorSquare.HEIGHT);
                        developmentLab.setBounds(inset, tradeY + (2 * (lineH + space)), dcW, lineH);
                        developmentSq.setBounds(inset + dcW, tradeY + (2 * (lineH + space)), ColorSquare.WIDTH, ColorSquare.HEIGHT);
                        resourceLab.setBounds(inset, tradeY + (3 * (lineH + space)), dcW, lineH);
                        resourceSq.setBounds(inset + dcW, tradeY + (3 * (lineH + space)), ColorSquare.WIDTH, ColorSquare.HEIGHT);
                    } else {
                        knightsLab.setBounds(inset + 2 * space, tradeY + lineH + space, knightsW, lineH);
                        knightsSq.setBounds(inset + 2 * space + knW, tradeY + lineH + space, ColorSquare.WIDTH, ColorSquare.HEIGHT);
                    }
                    roadLab.setBounds(inset + dcW + 3 * ColorSquare.WIDTH, tradeY + lineH + space, roadsW, lineH);
                    roadSq.setBounds(inset + dcW + roadsW + 3 * ColorSquare.WIDTH, tradeY + lineH + space, ColorSquare.WIDTH, ColorSquare.HEIGHT);
                    settlementLab.setBounds(inset + dcW + 3 * ColorSquare.WIDTH, tradeY + (2 * (lineH + space)), roadsW, lineH);
                    settlementSq.setBounds(inset + dcW + roadsW + 3 * ColorSquare.WIDTH, tradeY + (2 * (lineH + space)), ColorSquare.WIDTH, ColorSquare.HEIGHT);
                    cityLab.setBounds(inset + dcW + 3 * ColorSquare.WIDTH, tradeY + (3 * (lineH + space)), roadsW, lineH);
                    citySq.setBounds(inset + dcW + roadsW + 3 * ColorSquare.WIDTH, tradeY + (3 * (lineH + space)), ColorSquare.WIDTH, ColorSquare.HEIGHT);
                } else {
                ...
                if(client.devMode){
	                save.setBounds(dim.width - (3*bbW + space + inset), tbY, bbW, lineH);
	                load.setBounds(dim.width - (3*bbW + space + inset), tbY -lineH , bbW, lineH); //one line above
	                sim.setBounds(inset, tbY -lineH , bbW, lineH);
                }
                //option of muting sound during development
                muteLab.setBounds(inset, tradeY-(lineH + space), giveW, lineH); //above the give label
                muteSound.setBounds(inset + giveW + space, tradeY-(lineH + space), ColorSquare.WIDTH, ColorSquare.HEIGHT); //above restart/done button
                ...
                //---MG
                // offerBut.bounds()
                //registerTradeBut.setBounds(inset - 2, tradeY - 2, 150, 80);
                registerTradeBut.setBounds(inset - 2, tradeY - 2, 150, 40);
                bankTradeBut.setBounds(inset - 2, tradeY + 40 - 2, 150, 40);
                updateRegisterTradeBut();
            }
            else
            {
...
//                int sqW = fm.stringWidth(speakingQueueLab.getText().replace(' ','_'));  //Bug in stringWidth
//                speakingQueueLab.setBounds(inset + faceW + inset + 60, (inset + faceW) - lineH, vpW, lineH);
//                speakingQueueSq.setBounds(inset + faceW + inset + sqW + space + 56, (inset + faceW) - lineH, ColorSquare.WIDTH, ColorSquare.HEIGHT);
////                System.err.println("position of speaker " + player.getName() + " index: " + game.speakingQueue.indexOf(player.getName()));
                ...
                //if fully observable, display the player's dev cards and hidden resources
                if(gp!=null && gp.fullyObservable){
                    Dimension sqpDim = sqPanel.getSize();
                    int cardsH = 5 * (lineH + space);
                	int sheepW = fm.stringWidth("Sheep:_");
	                int tradeH = sqpDim.height + space + (2 * (lineH + space));
	                int sectionSpace = (dim.height - (inset + faceW + cardsH + tradeH + lineH + inset)) / 3;
	                int tradeY = inset + faceW + sectionSpace;
	                int cardsY = tradeY + tradeH + sectionSpace;
	                ...
	                clayLab.setBounds(inset, cardsY - (lineH + space), sheepW, lineH);
	                claySq.setBounds(inset + sheepW + space, cardsY - (lineH + space), ColorSquare.WIDTH, ColorSquare.HEIGHT);
	                oreLab.setBounds(inset, cardsY , sheepW, lineH);
	                oreSq.setBounds(inset + sheepW + space, cardsY, ColorSquare.WIDTH, ColorSquare.HEIGHT);
	                sheepLab.setBounds(inset, cardsY + (lineH + space), sheepW, lineH);
	                sheepSq.setBounds(inset + sheepW + space, cardsY + (lineH + space), ColorSquare.WIDTH, ColorSquare.HEIGHT);
	                wheatLab.setBounds(inset, cardsY + (2 * (lineH + space)), sheepW, lineH);
	                wheatSq.setBounds(inset + sheepW + space, cardsY + (2 * (lineH + space)), ColorSquare.WIDTH, ColorSquare.HEIGHT);
	                woodLab.setBounds(inset, cardsY + (3 * (lineH + space)), sheepW, lineH);
	                woodSq.setBounds(inset + sheepW + space, cardsY + (3 * (lineH + space)), ColorSquare.WIDTH, ColorSquare.HEIGHT);
    ...
    public void passiveMode() {    	
    	...
    	// hide the player send checkboxes as well
    	for (ColorSquare cs : playerSend) {
    		remove(cs);
    	}
