 ...
 */
public class SOCPlayerNumbers implements Serializable
{
    /**
        ...
        for (int i = 0; i < landHexCoords.length; i++)
        {
            if (landHexCoords[i] != robberHex)
            {
                ...
                while (pairsEnum.hasMoreElements())
                {
                    ...
                    if (pair.getA() == number)
                    {
                        resources.addElement(Integer.valueOf(pair.getB()));
                    }
                }
            }
        }
    ...
    public void addNumberForResource(int number, int resource, int hex)
    {
        ...
        {
            numbersForResource[resource].addElement(Integer.valueOf(number));
