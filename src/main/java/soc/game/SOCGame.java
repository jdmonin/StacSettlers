...
public class SOCGame implements Serializable, Cloneable
{
    ...
    public static final int PLAY1_LEGACY = 21; // Special game state to be treated as PLAY1 by legacy agents, or ignored
     ...
     */
    private int vpWinner = 10;

    /**
     * 
     */
    private boolean isTrainingGame = false;
    ...
    /**
     * When to stop the game if it hasn't finished by then.
     */
    private int finishTurn = Integer.MAX_VALUE;
    ...
    /**
     * keeping track of which dev cards have been played.
     */
    private int[] playedDevCards;
    ...
    /**
     * ---MG
     * The queue for requests to speak with the SOCPlayerInterface:textInput field 
     */
    public ArrayList<String> speakingQueue = new ArrayList<String>();
    
    /**
     * ---MG
     * The person currently speaking, i.e. the one with the enabled textInput in SOCPlayerInterface 
     */
    public String currentSpeaker;
    
    /**
     * Monitoring whether a 7 has been rolled at some point in this game. (For evaluation purposes.)
     */
    public boolean sevenHasBeenRolled;
        ...
        throws IllegalArgumentException
    {
        ...
        sevenHasBeenRolled = false;
        
        //based on type as in SOCDevCardConstants
        playedDevCards = new int[5];
        Arrays.fill(playedDevCards, 0);
    }

    /**
     * update the tracking of dev cards based on the one just played;
     * @param type the dev card type which has been played/observed
     */
    public void devCardPlayed(int type){
		if(type >= SOCDevCardConstants.UNKNOWN){
			return;
		}
		
		if(type>= SOCDevCardConstants.CAP)
			type = SOCDevCardConstants.CAP;
		
		playedDevCards[type]++;
    }
    
    public int[] getPlayedDevCardAmount(){
    ...
    /**
     * This setter's intended use is only for renaming the clone after the game in progress.
     * Avoid using in other scenarios as game information in SOCGameList is accessed via the game's name.
    ...
    public void setVpWinner(int newVpWinner) {
        vpWinner = newVpWinner;
    }

    /**
     * Getting the VP needed to win this game
     * @return 
     */
    public int getVpWinner() {
        return vpWinner;
    }

    public void setIsTrainingGame(boolean flag) {
        isTrainingGame = flag;
    }
    ...
    public void setCurrentPlayerNumber(int pn)
    {
        ...
        if ((pn >= -1) && (pn < players.length))
        {
            ...
            if ((pn >= 0) && (players[pn].getTotalVP() >= vpWinner))
                checkForWinner();
        }
    }
    ...
    /**
     * 
     * @return the old game state without checking for game reset requests
     */
    ...
    {
    	if(loadBoard){
    		//do the logic for reading the file and loading.
    		String fileName = "saves/board/_" + SOCBoard.class.getName();
    		File f = new File(fileName + ".dat");
    		if(f.exists())
    			board = (SOCBoard)DeepCopy.readFromFile(fileName);
    		else{
    			D.ebugERROR("Can't find the saved board layout, creating a new one");
    			board.makeNewBoard(opts, false);
    			}
    	}else
    		board.makeNewBoard(opts, false);
    ...
    public void endTurn()
    {
        ...
        if (players[currentPlayerNumber].getTotalVP() >= vpWinner)
    ...
    public void updateAtTurn()
    {
        ...
        if (gameState == PLAY)
        {
            ...
            if(turnCount == finishTurn)
            	gameState = OVER;//end game earlier if a turn limit is set
    ...
    /**
     * calculate how many resources have been prevented from dropping during this roll and add to the previous values
     *
     * @param roll the roll
     * @param the previous number of resources blocked before this roll
     *
     * @return the number of resources blocked by the current position of the robber
     */
    public int[] getResourcesBlocked(int roll, int[] previousValues){
    	final int robberHex = board.getRobberHex();
    	if(board.getHexTypeFromCoord(robberHex) != SOCBoard.DESERT_HEX && board.getHexNumFromCoord(robberHex) == roll){
	    	int[] nodes;
	    	try {
	    		nodes = board.getAdjacentNodesToHexOld(robberHex);
			} catch (Exception e) {
				D.ebugFATAL(e,"Cannot get adjacent nodes to hex, wrong coordinate or direction");
				return previousValues; //do not modify the values if an error
			}
	    	//modify the values based on the surrounding pieces
	    	for(int i : nodes){
	    		SOCPlayingPiece piece = board.settlementAtNode(i);
	    		if(piece!=null){
	    			if(piece.getType() == SOCPlayingPiece.SETTLEMENT){
	    				previousValues[piece.getPlayer().getPlayerNumber()]++;
	    			}
	    			else if(piece.getType() == SOCPlayingPiece.CITY){
	    				previousValues[piece.getPlayer().getPlayerNumber()]=+2;
	    			}
	    		}
	    	}
    		return previousValues;//return the changed values
    	}
    	else 
    		return previousValues; //return the unchanged values because the hex number is not equal to the roll
    }
    
    /**
     * @return total number of resources that can be blocked per turn 
     */
    public int getTotalPossibleBlockedRss(){
    	final int robberHex = board.getRobberHex();
    	int value = 0;
    	int[] nodes;
    	try {
    ...
    /**
     * Used by MCTS rewarding logic for normalising values of total resources stolen.
     * @return an array with the number of resources that can be stolen for each player per turn.
     */
    public int[] getPossibleBlockedRss(){
    	final int robberHex = board.getRobberHex();
    	...
    	try {
			...
			D.ebugFATAL(e,"Cannot get adjacent nodes to hex, wrong coordinate or direction");
			return values; //do not modify the values if an error
		}
    	//modify the values based on the surrounding pieces
    	for(int i : nodes){
    		SOCPlayingPiece piece = board.settlementAtNode(i);
    		if(piece!=null){
    			if(piece.getType() == SOCPlayingPiece.SETTLEMENT){
    				values[piece.getPlayer().getPlayerNumber()]++;
    			}
    			else if(piece.getType() == SOCPlayingPiece.CITY){
    				values[piece.getPlayer().getPlayerNumber()]=+2;
    			}
    		}
    	}
		return values;//return the changed values
    }
    ...
    public int stealFromPlayer(int pn)
    {
        ...
        if(cnt==0)
        	return -1;//when expanding the tree in MCTS we are using the robot's belief of the other players resources. This handles the case when a player has no rss and we think it does.
     ...
     *
     * @return null if the two players can make the trade
     *         described in the offering players current offer;
     *         otherwise a string describing the reason for refusing the trade
     *
     ...
     */
    public String canMakeTrade(int offering, int accepting)
    {
        ...
        if (D.ebugIsEnabled()) {
            ...
            for (int i = 0; i < players.length; i++) {
                offerString += "\n   " + i + ": ";
                SOCTradeOffer curOffer = players[i].getCurrentOffer();
                offerString += (curOffer == null) ? "NULL" : curOffer.toString();
            }
            D.ebugPrintlnINFO("*** current offers: " + offerString);
        }
        
        if (gameState != PLAY1)
        {
            return "Trading not allowed at the moment.";
        }

        if (isGameOptionSet("NT"))
            return "No trades allowd in this game.";

        if (players[offering].getCurrentOffer() == null)
        {
            return "Can't identify the current trade offer.";
        }

        if ((currentPlayerNumber != offering) && (currentPlayerNumber != accepting))
        {
            return "Only the current player can trade.";
        }
        ...
        //---MG we're allowing parital offers
...
//        }
        ...
        {
            return offeringPlayer.getName() + " does not have the promised resources.";
        }
        ...
        {
            return acceptingPlayer.getName() + " does not have the promised resources.";
        }
        
        //game rules state that no resources may be given away for free.
        //as this method is called when finalizing a trade we should check for number of resources for both sets is higher than 0
        if(offer.getGetSet().getTotal()==0 || offer.getGiveSet().getTotal()==0)
        	return "Resources cannot be given away for free.";

        //game rules don't allow exchanging like for like resources
        if (!offer.getGetSet().disjoint(offer.getGiveSet()))
            return "Can't trade like for like resources.";
     ...
     * If they have 2 or more roads and there are at least two potentials, may place 2; gameState becomes PLACING_FREE_ROAD1.
     * If they have just 1 road or less than two potentials, may place that; gameState becomes PLACING_FREE_ROAD2.
     * If they have 0 roads, cannot play the card, so it never gets here.
     * TODO: this decision would break the logic in the SOCRobotBrain run loop if the heuristics agents decide to play road building card when they can only place one road
    ...
    public void playRoadBuilding()
    {
        ...
        players[currentPlayerNumber].numRBCards++;
        oldGameState = gameState;
        if (players[currentPlayerNumber].getNumPieces(SOCPlayingPiece.ROAD) > 1 && players[currentPlayerNumber].hasTwoPotentialRoads())
        {
    ...
    public void checkForWinner()
    {
        ...
        //special treatment of training games for our evaluation setup
        ...
        if ((pn >= 0) && (pn < maxPlayers)
            && (players[pn].getTotalVP() >= vpWinner))
        {
            gameState = OVER;
            playerWithWin = pn;
        }
    ...
    /**
     * @return an array containing the players' names in the same order as their positioning on the board
     */
    ...
    /**
     * Rename all players. 
     * Note: if this is not in the correct order or does not contain the correct names, 
     * some robots will not receive anymore messages from the server as the StringConnections are created
     * using the player's name instead of number and it gets the correct order dinamically from the
     * game object.
     * 
     * @param names the new player names (must be in the correct order)
     */
    ...
    /**
     * Sets the turn number when the game should stop if it hasn't already finished by then. 
     * @param turn number when it should stop the game play. (if turn<= 0) then stops only when the preset no. of VP is reached by one player
     */
    public void setFinishTurn(int turn){
    	if(turn > 0)	
    		finishTurn = turn;
    }
    ...
    /**
     * Method for updating time started and last action time when loading a previous game.
     */
    ...
    /**
     * reseeds the random number generator to avoid turning the game into a non-stochastic one when a robot is loading.
     */
    public void resetRandom(){
    	rand.setSeed(System.currentTimeMillis());
    }
    /**
     * Method for transforming the observable game state into a feature vector containing the same information as a row inside the observable states
     * table in the DB.
     * @return
     */
    public ObsGameStateRow turnCurrentStateIntoOGSR(){
    	ObsGameStateRow ogsr = new ObsGameStateRow(0, getName());
        SOCBoard board = getBoard();
        ...
        ogsr.setDevCardsLeft(getNumDevCards());
        ogsr.setStartingPlayer(getFirstPlayer());
        ogsr.setCurrentPlayer(getCurrentPlayerNumber());
        ogsr.setPlayedDevCard(getPlayer(getCurrentPlayerNumber()).hasPlayedDevCard());
        ogsr.setDiceResult(getCurrentDice());
        ...
        for (SOCPlayer p : getPlayers()) {
          ...
          //Newly bought Dev cards
            ...
            ogsr.setPlayedRB(pn, p.numRBCards);
            ogsr.setPlayedMono(pn, p.numMONOCards);
            ogsr.setPlayedDisc(pn, p.numDISCCards);
            
            //Total VPs
            ogsr.setTotalVP(pn, p.getTotalVP());
            
            //Public VPs
            ogsr.setPublicVP(pn, p.getPublicVP());
            
            //labels:LA,LR
            ogsr.setLALabel(pn, (p.hasLargestArmy()) ? 1 : 0);
            ogsr.setLRLabel(pn, (p.hasLongestRoad()) ? 1 : 0);
            
            //Resources currently in hand (an array of numbers: clay, ore, sheep, wheat, wood, unknown)
            SOCResourceSet rs = p.getResources();
            int[] rss = new int[]{rs.getAmount(SOCResourceConstants.CLAY),rs.getAmount(SOCResourceConstants.ORE),rs.getAmount(SOCResourceConstants.SHEEP), 
            		rs.getAmount(SOCResourceConstants.WHEAT), rs.getAmount(SOCResourceConstants.WOOD), rs.getAmount(SOCResourceConstants.UNKNOWN)};
            ogsr.setResources(pn, rss);
            
            //Pieces left to build (array of numbers: roads, settlements, cities) 
            ogsr.setRoadsLeftToBuild(pn, p.getNumPieces(SOCPlayingPiece.ROAD));
            ogsr.setSettlementsLeftToBuild(pn, p.getNumPieces(SOCPlayingPiece.SETTLEMENT));
            ogsr.setCitiesLeftToBuild(pn, p.getNumPieces(SOCPlayingPiece.CITY));
            
            //Access to what numbers on the board
            SOCPlayerNumbers playerNumbers = p.getNumbers();
            int[][] tn = new int[5][];
            
            for(int j = SOCResourceConstants.MIN; j < SOCResourceConstants.MAXPLUSONE -1; j++){
            	Vector nums = playerNumbers.getNumbersForResource(j);
            	tn[j-1] = new int[nums.size()];
            	int index = 0;
            	for(Object o : nums){
            		tn[j-1][index] = (int) o;
            		index++;
            ...
            ogsr.setTouchingResourceTypes(pn, trt);
            ...
            ogsr.setTouchingPortTypes(pn, tpt);
        
        }
        
        //don't forget to set the pieces on board 
        ogsr.setPiecesOnBoard(pob);
        ogsr.resolveNumbersDimensionality();
