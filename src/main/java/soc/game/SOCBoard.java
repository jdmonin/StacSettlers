...
{
	/**
	 * list of illegal coordinates which are not caught by the methods returning adjacent edges....
	 * these are in decimal so can be used straight away.
	 */
	public static final int[] illegalEdges_V1 = {18,20,22,33,40,65,74,97,108,130,141,164,173,198,205,216,218,220};
    //
    // Hex types
    //
    ...
    /**
     * Create the default Settlers of Catan board for 4 players based on the SOC manual.
     */
        ...
        throws IllegalArgumentException
    {
        ...
        if (is6player)
        {
            for (i = 0x07; i <= 0x6D; i += 0x11)
                nodesOnBoard.put(Integer.valueOf(i), t);
        }
     ...
     */
    public void makeNewBoard(Hashtable opts, boolean defaultPorts)
    {
        ...
        //default board port configuration
        int[] portHex = { 0, SHEEP_PORT, 0, 0, CLAY_PORT, WOOD_PORT, 0, WHEAT_PORT, ORE_PORT };
        if(!defaultPorts){
        	//else copy and shuffle the ports, and check vs game option BC
        	final int[] portTypes = (is6player) ? PORTS_TYPE_V2 : PORTS_TYPE_V1;
	        portHex = new int[portTypes.length];
	    	System.arraycopy(portTypes, 0, portHex, 0, portTypes.length);
                //don't make this call to get the standard map
        ...
        */
        ...
        if (is6player)
        {
            for (int i = 0; i < PORTS_FACING_V2.length; ++i)
            {
                ...
                ports[ptype].addElement(Integer.valueOf(nodes[0]));
                ports[ptype].addElement(Integer.valueOf(nodes[1]));
            }            
        } else {
        ...
        ports[portHex[0]].addElement(Integer.valueOf(0x27));  // Port touches the upper-left land hex, port facing SE
        ports[portHex[0]].addElement(Integer.valueOf(0x38));  // [port's hex is NW of the upper-left land hex]

        ports[portHex[1]].addElement(Integer.valueOf(0x5A));  // Touches middle land hex of top row, port facing SW
        ports[portHex[1]].addElement(Integer.valueOf(0x6B));  // [The port hex itself is 2 to right of prev port hex.]

        ports[portHex[2]].addElement(Integer.valueOf(0x9C));  // Touches rightmost land hex of row above middle, SW
        ports[portHex[2]].addElement(Integer.valueOf(0xAD));

        ports[portHex[3]].addElement(Integer.valueOf(0xCD));  // Rightmost of middle-row land hex, W
        ports[portHex[3]].addElement(Integer.valueOf(0xDC));

        ports[portHex[4]].addElement(Integer.valueOf(0xC9));  // Rightmost land hex below middle, NW
        ports[portHex[4]].addElement(Integer.valueOf(0xDA));

        ports[portHex[5]].addElement(Integer.valueOf(0xA5));  // Port touches middle hex of bottom row, facing NW
        ports[portHex[5]].addElement(Integer.valueOf(0xB6));  // [The port hex itself is 2 to right of next port hex.]

        ports[portHex[6]].addElement(Integer.valueOf(0x72));  // Leftmost of bottom row, NE
        ports[portHex[6]].addElement(Integer.valueOf(0x83));

        ports[portHex[7]].addElement(Integer.valueOf(0x43));  // Leftmost land hex of row below middle, E
        ports[portHex[7]].addElement(Integer.valueOf(0x52));

        ports[portHex[8]].addElement(Integer.valueOf(0x25));  // Leftmost land hex above middle, facing E
        ports[portHex[8]].addElement(Integer.valueOf(0x34));
        */

    }

    /**
     *generates the board land hex configuration and numbers for the 4 player game as per the SOC manual.
     */
    private void setDefaultLandHex(){
        //standard map layout from Rules book
        //0: desert, 1: clay, 2: ore, 3: sheep, 4: wheat, 5: wood
        final int[] landHex = { 2, 4, 5, 2, 4, 3, 4, 3, 5, 1, 0, 1, 3, 3, 5, 1, 2, 5, 4 };
        final int[] numPath = { 29, 30, 31, 26, 20, 13, 7, 6, 5, 10, 16, 23, 24, 25, 19, 12, 11, 17, 18 };
        final int[] numbers = { 5, 2, 6, 3, 8, 10, 9, 12, 11, 4, 8, 10, 9, 4, 5, 6, 3, 11 };    	
        ...
        for (int i = 0; i < landHex.length; i++)
        {
            ...
            {
                // place the numbers
                numberLayout[numPath[i]] = numbers[cnt];
                cnt++;
            }
        }  // for(i in landHex)
    }    
        ...
        (int[] landHex, final int[] numPath, final int[] number, SOCGameOption optBC)
    {
        ...
        do   // will re-do placement until clumpsNotOK is false
        {
            ...
            if (checkClumps)
            {
                ...
                for (int i = 0; i < landHex.length; ++i)
                {
                    unvisited.addElement(Integer.valueOf(numPath[i]));
                }
                ...
                while (unvisited.size() > 0)
                {
                    ...
                    for (int i = 0; i < adjacent.size(); ++i)
                    {
                        ...
                        if (resource == hexLayout[adjIdx])
                        {
                            ...
                            if (clump == null)
                                clump = new Vector();
                            Integer adjIdxObj = Integer.valueOf(adjIdx);
    ...
    public void setHexLayout(int[] hl)
    {
        ...
        //since we are overwriting stuff; reinitialise the port vector just to be sure
        /**
         * initialize the port vector
         */
        ports = new Vector[6];  // 1 per resource type, MISC_PORT to WOOD_PORT
    ...
    /**
     * @param coord  Edge coordinate for the 4-player encoding.
     * @return a Vector[4] for each direction, the first edge plus the two connecting in continuation, so we can check later if two roads can be placed
     */
    public static Vector[] get2AdjacentEdgesToEdge_v1(int coord)
    {
        //there are 4 possible coords so this will return a Vector[4]
    	Vector[] edges2 = new Vector[4];
		for(int i = 0; i < 4; i++){
			edges2[i] = new Vector();
		}
        int tmp, tmp2;
        ...
        if ((((coord & 0x0F) + (coord >> 4)) % 2) == 0)
        {
        	//this is "\"
            tmp = coord - 0x10;
            if(isEdgeOnBoard_v1(tmp))
	            if ((tmp >= MINEDGE_V1) && (tmp <= MAXEDGE_V1))
	            {
	            	edges2[0].addElement(Integer.valueOf(tmp));
	            	//this is "|" in continuation
	            	tmp2 = tmp - 0x10;
	                if ((tmp2 >= MINEDGE_V1) && (tmp2 <= MAXEDGE_V1) && isEdgeOnBoard_v1(tmp2))
	                	edges2[0].addElement(Integer.valueOf(tmp2));
            ...
            if(isEdgeOnBoard_v1(tmp))
	            if ((tmp >= MINEDGE_V1) && (tmp <= MAXEDGE_V1))
	            {
	            	edges2[1].addElement(Integer.valueOf(tmp));
	            	//this is "|" in continuation
	            	tmp2 = tmp + 0x01;
	                if ((tmp2 >= MINEDGE_V1) && (tmp2 <= MAXEDGE_V1) && isEdgeOnBoard_v1(tmp2))
	                	edges2[1].addElement(Integer.valueOf(tmp2));
            ...
            if(isEdgeOnBoard_v1(tmp))
	            if ((tmp >= MINEDGE_V1) && (tmp <= MAXEDGE_V1))
	            {
	            	edges2[2].addElement(Integer.valueOf(tmp));
	            	//this is "/" in continuation
	            	tmp2 = tmp + 0x11;
	                if ((tmp2 >= MINEDGE_V1) && (tmp2 <= MAXEDGE_V1) && isEdgeOnBoard_v1(tmp2))
	                	edges2[2].addElement(Integer.valueOf(tmp2));
            ...
            if(isEdgeOnBoard_v1(tmp))
	            if ((tmp >= MINEDGE_V1) && (tmp <= MAXEDGE_V1))
	            {
	            	edges2[3].addElement(Integer.valueOf(tmp));
	            	//this is "\" in continuation
	            	tmp2 = tmp - 0x11;
	                if ((tmp2 >= MINEDGE_V1) && (tmp2 <= MAXEDGE_V1) && isEdgeOnBoard_v1(tmp2))
	                	edges2[3].addElement(Integer.valueOf(tmp2));
    ...
    /**
     * @param coord  Edge coordinate; for the 4-player encoding only.
     * @return the adjacent edges to this edge, as a Vector of Integer coordinates
     */
    public static Vector getAdjacentEdgesToEdge_v1(int coord)
    {
        ...
        Vector legalEdges = new Vector(4);
        
        for(Object o : edges){
        	if(isEdgeOnBoard_v1((int) o))
        		legalEdges.addElement(Integer.valueOf((int)o));
        }
     ...
     * @return
     */
    public static boolean isEdgeOnBoard_v1(int coord){
    	for(int illCoord : illegalEdges_V1)
    		if(coord == illCoord)
    			return false;
    	return true;
    }
     ...
     * @return ana array containing the coordinates of the adjacent nodes to the given node coordinate
     */
    public static int[] getAdjacentNodesToNode_arr_v1(int coord)
    {
        int nodes[] = new int[3];
        for (int i = 0; i < 3; ++i)
            nodes[i] = getAdjacentNodeToNode_v1(coord, i);

        return nodes;
    }

    /**
     * Utility to get the adjacent node to another node given coordinate and direction
     * @param nodeDir direction to look at
     * @param nodeCoord coordinate of the node
     * @return coordinate of the adjacent node
     */
    public static int getAdjacentNodeToNode_v1(final int nodeCoord, final int nodeDir)
    {
        ...
        case 0:  // NW or SW
            tmp = nodeCoord - 0x11;
            if ((tmp >= MINNODE_V1) && (tmp <= MAXNODE) && ((nodeCoord & 0x0F) > 0))
                node = tmp;
            else
                node = -9;
            break;
        ...
        case 1:  // NE or SE
            tmp = nodeCoord + 0x11;
            if ((tmp >= MINNODE_V1) && (tmp <= MAXNODE) && ((nodeCoord & 0x0F) < 0xD))
                node = tmp;
            else
                node = -9;
            break;
        ...
        case 2:  // N or S
            ...
            if (((nodeCoord >> 4) % 2) == 0)
            {
                ...
                if (hasSouthernEdge && (tmp >= MINNODE_V1) && (tmp <= MAXNODE))
                    node = tmp;
                else
                    node = -9;
            }
            else
            {
                ...
                boolean hasNorthernEdge = (nodeCoord < 0x18) || (nodeCoord > 0x7E)
                  || (0 != ((nodeCoord - 0x18) % 0x22));
                tmp = nodeCoord - 0x10 + 0x01;
                if (hasNorthernEdge && (tmp >= MINNODE_V1) && (tmp <= MAXNODE))
                    node = tmp;
                else
                    node = -9;
            }
    ...
    /**
     * Get all nodes two spots away from the requested node.  Wrapper to the getAdjacentNodeToNode2Away which considers all directions   
     * @param nodeCoord
     * @return
     */
    public List<Integer> getAdjacentNodesToNode2Away(final int nodeCoord) {
    	List<Integer> ret = new ArrayList<Integer>();
    	for (int i=1; i<7; i++) {
    		int n = getAdjacentNodeToNode2Away(nodeCoord, i);
    		if (n>0) {
    ...
    public int[] getAdjacentNodesToHexOld(final int hexCoord) throws IllegalArgumentException{
    	...
    	for(int i=0; i < 6; i++){
    		nodes[i] = getAdjacentNodeToHex(hexCoord, i);
    	}
    	return nodes;
    }
     ...
     * @return an array of node coordinates surrounding a hex coordinate
     */
    public static int[] getAdjNodesToHex(final int hexCoord) throws IllegalArgumentException{
    	int[] nodes = new int[6];//always 6 nodes adjacent to a hex
    	int[] distances = { 0x01, 0x12, 0x21, 0x10, -0x01, -0x10 };//taken from HEXNODES
    	for(int i=0; i < 6; i++){
    		nodes[i] = hexCoord + distances[i];
    	}
    	return nodes;
    }
    ...
    public boolean isNodeOnBoard(int node)
    {
        if (node < 0)
            return false;
        return nodesOnBoard.containsKey(Integer.valueOf(node));
    }

    /**
	 * Check if a node is on board for the 4 player encoding.
     * @return true if the node is on the land of the board (not water)
     * @param node Node coordinate
     */
    public static boolean isNodeOnBoard_v1(int coord)
    {
        if (coord < 0)
            return false;
        if (coord < MINNODE_V1)
