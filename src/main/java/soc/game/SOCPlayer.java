...
public class SOCPlayer implements SOCDevCardConstants, Serializable, Cloneable
{
    ...
    private List<Integer> legalSettlementsList = null;
    
    public boolean isLegalSettlement(int i) {
        return legalSettlements[i];
    }
    
    /**
     * Return the list of legal settlements (ie nothing there currently,
     * not blocked by an adjacent settlement
     * @return
     */
    public List<Integer> getLegalSettlements() {
    	if (legalSettlementsList==null) {	    	
	    	legalSettlementsList = new ArrayList<Integer>(54); // 54 potential building spots at beginning of game
	    	for (int i=0; i<legalSettlements.length; i++) {
	    		if (legalSettlements[i]) {
	    			legalSettlementsList.add(Integer.valueOf(i));
	    		}
	    	}
    	}
    	return legalSettlementsList;
    }
    ...
    /**
     * Method intended for fooling the server into thinking this player is a human until simulations finish so it won't end its turn.
     * DO NOT use for anything else, as it might break the existing logic
     * 
     * @param flag to true or false;
     */
    public void setRobotFlagUnsafe(boolean flag){
    	robotFlag = flag;
    }
    ...
    public void updatePotentials(SOCPlayingPiece piece)
    {
        ...
        case SOCPlayingPiece.SETTLEMENT:
            ...
            else
            {
                ...
                Set<Integer> ourRoads = new HashSet<Integer>();
    ...
    /**
     * Required for deciding on the game state given a road building card is beeing played
     * @return true if there are either two potential roads or two potential chained roads
     */
    public boolean hasTwoPotentialRoads()
    {
    	// check if there are two potential roads not necessarily connected first
    	 int count = 0;
    	 int coord = -1;
    	for (int i = game.getBoard().getMinNode(); i <= SOCBoard.MAXNODE; i++)
         {
    	...
    	//if there is only one road, check if there is one potential road connected to it
    	if(count == 1){
    		SOCRoad tmpRoad = new SOCRoad(this, coord, null);
    		game.putTempPiece(tmpRoad);
    		if(this.hasPotentialRoad()){ 
     ...
     * @param ga the new game object 
     */
    public void setGame(SOCGame ga){
    	game = ga;
    }
