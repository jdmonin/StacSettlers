...
public abstract class SOCMessage implements Serializable, Cloneable
{
    ...
    /** @since 1.1.07 */
    public static final int NEWGAMEWITHOPTIONSREQUEST = 1078;  // gameoptions, 20090601c.put(SOCGameCopy.toCmd(mes.getGame()));
    ...
    /** ---MG @since STAC version */
    public static final int REQUESTTOSPEAK = 2000; // user sends request to speak or withdraw the request

    /** ---MG @since STAC version */
    public static final int PERMISSIONTOSPEAK = 2001; // server sends permission to speak to a user

    /** ---MG @since STAC version */
    public static final int SPEAKINGQUEUECHANGED = 2002; // server sends users the changed state of the speaking queue

    /** ---MG @since STAC version */
    public static final int PLAYERSTARTSTRADING = 2003; // a player starts a trading interaction, which is printed to the interaction history so the other players know what's happening

    /** ---MG @since STAC version */
    public static final int CLEARGAMEHISTORY = 2004; // sent by the server to instruct clients to clear their game interaction history
    ...
    /////////////////////////////////////////
    //---MD (messages for save/load mechanism)
    public static final int GAMECOPY = 2005;
    public static final int LOADGAME = 2006;
    public static final int ROBOTFLAGCHANGE = 2007;
    
    /** ---MG @since STAC version 
     * Message containing the result of calling the parser. Sent by the server to the robot clients.
     */
    public static final int PARSERESULT = 2008;
    public static final int COLLECTDATA = 2009;

    /** ---MG @since STAC version 
     * Messages between server and player client for confirming a trade before it's executed.
     */
    ...
    public static SOCMessage toMsg(String s)
    {
	    ...
	    case GAMESWITHOPTIONS:       // per-game options, 20090601, v1.1.07
            ...
            case REQUESTTOSPEAK: // player requests to speak or withdraws the request
            	return SOCRequestToSpeak.parseDataStr(data);
            	
            case PERMISSIONTOSPEAK: // server sends the permission to speak for a player
            	return SOCPermissionToSpeak.parseDataStr(data);

            case SPEAKINGQUEUECHANGED: // server sends the new state of the speaking queue to the players
            	return SOCSpeakingQueueChanged.parseDataStr(data);
            	
            case PLAYERSTARTSTRADING: // a player starts a trading interaction, which is printed to the interaction history so the other players know what's happening
            	return SOCPlayerStartsTrading.parseDataStr(data);
            	
            case CLEARGAMEHISTORY: // the server requests all clients to clear the game interaction history
            	return SOCClearGameHistory.parseDataStr(data);
            	
            case GAMECOPY: //requests cloning of game states
            	return SOCGameCopy.parseDataStr(data);
            	
            case LOADGAME: //load a game using the saved states
            	return SOCLoadGame.parseDataStr(data);
            
            case ROBOTFLAGCHANGE:// change the player's robot flag
            	return SOCRobotFlag.parseDataStr(data);
           
            case PARSERESULT: //send the output of a call to the parser to the (robot) clients
                return SOCParseResult.parseDataStr(data);
            
            case COLLECTDATA: //triggers the collection of data
                return SOCCollectData.parseDataStr(data);
                
            case CONFIRMTRADETREQUEST: //server asks SOCPlayerClient for confirmation before executing a trade
                return StacConfirmTradeRequest.parseDataStr(data);
     ...
     * Note: This is just a generic function; for messages ACTUALLY passed to the parser, the message class must overwrite this method.
...
//
//            //set the class name as the only node
